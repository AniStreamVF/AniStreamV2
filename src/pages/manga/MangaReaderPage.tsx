...
import { DARK_THEME_VARS } from "@/lib/themeConstants";
...
const READER_SETTINGS_KEY = "AniStream:manga-reader-settings:v1";
...
function formatLanguageLabel(value: string | null | undefined): string {
  ...
  if (key === "jp") return "Japonais";
  if (key === "kr") return "Coréen";
  if (key === "zh") return "Chinois";
  if (key === "en") return "Anglais";
...
function normalizePreferredMangaLanguage(value: string | null | undefined): string {
  ...
  if (normalized === "fr" || normalized === "jp" || normalized === "kr" || normalized === "zh" || normalized === "en") {
    return normalized;
  }
  return "auto";
}
...
export default function MangaReaderPage() {
  ...
  const autoLanguageLabel =
    profilePreferredMangaLanguage === "auto"
      ? "toutes les langues"
      : `Paramètre : ${formatLanguageLabel(profilePreferredMangaLanguage)}`;

  const chapterLabel =
    currentChapter?.chapterTitle || (chapterNumber != null ? `Chapitre ${chapterNumber}` : chapterKey);
  ...
  if (contentSafetySettings.warnBeforeAdultOpen && !allowAdultForSession && loadingMangaMeta) {
    return (
      <div style={DARK_THEME_VARS} className="min-h-screen bg-background flex flex-col items-center justify-center">
        {!isNative && <Background />}
  ...
  if (requiresAdultWarning) {
    return (
      <div style={DARK_THEME_VARS} className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
        {!isNative && <Background />}
        <GlassPanel className="w-full max-w-2xl p-6 md:p-8 border border-amber-500/30">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black">Avertissement de contenu sensible</h2>
              <p className="mt-2 text-muted-foreground">
                Ce chapitre appartient à un titre mature. Continuez uniquement si vous êtes à l'aise
                avec du contenu 18+.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            >
              Toujours afficher les médias matures
            </button>
          </div>
        </GlassPanel>
      </div>
  ...
  if (error || !readData || pages.length === 0) {
    ...
    return (
      <div style={DARK_THEME_VARS} className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        {!isNative && <Background />}
        <GlassPanel className="w-full max-w-2xl p-6 md:p-8 border border-white/10 text-left">
          <h2 className="text-xl font-bold mb-3">Impossible de charger le chapitre</h2>
          <p className="text-muted-foreground mb-3">
            {readData?.guidance?.message || "Nous n'avons pas pu lire ce chapitre. Il est peut-être indisponible ou supprimé."}
          </p>

          {attemptedProviders.length > 0 && (
            <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">
              Fournisseurs tentés : {attemptedProviders.join(", ")}
            </p>
          )}
  ...
  return (
    <div ref={readerRootRef} style={DARK_THEME_VARS} className="min-h-screen bg-black text-white">
      {shouldShowReaderHud && (
        <>
          <div className="sticky top-0 z-50 bg-black/85 backdrop-blur-md border-b border-white/10 p-3 md:p-4 flex items-center gap-3">
            ...
            >
              ...
              <span className="font-bold hidden sm:inline">Retour</span>
            </button>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] uppercase tracking-wider text-white/50">
                ...
                {formatLanguageLabel(activeSource?.language || readData.data?.chapter?.language)} • Chapitre{" "}
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <button
                ...
                className="p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"
                title="Activer/désactiver le plein écran (F)"
              >
        ...
        {readerSettings.readingMode === "paged" ? (
          <div className="relative min-h-[calc(100vh-9rem)] flex items-center justify-center px-2 md:px-8">
            <button
              ...
              title="Page précédente (Gauche/K)"
            >
            ...
            <button
              ...
              title="Page suivante (Droite/J/Espace)"
            >
      ...
      {isFullscreen && (
        <button
          ...
          title="Quitter le plein écran (F / Échap)"
        >
      ...
      {shortcutsOpen && (
        ...
        >
          <GlassPanel className="w-full max-w-lg p-5 border border-white/15" onClick={(event) => event.stopPropagation()}>
            <h3 className="text-lg font-bold mb-3">Raccourcis du lecteur</h3>
            <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
              <p>Droite / J / Espace</p><p className="text-right text-foreground">Page suivante</p>
              <p>Gauche / K</p><p className="text-right text-foreground">Page précédente</p>
              <p>[ / ]</p><p className="text-right text-foreground">Chapitre précédent / suivant</p>
              <p>S</p><p className="text-right text-foreground">Ouvrir les paramètres</p>
              <p>?</p><p className="text-right text-foreground">Afficher/masquer l'aide</p>
              <p>F</p><p className="text-right text-foreground">Plein écran</p>
              <p>M</p><p className="text-right text-foreground">Changer de mode de lecture</p>
              <p>H</p><p className="text-right text-foreground">Afficher/masquer le HUD</p>
              <p>+ / - / 0</p><p className="text-right text-foreground">Zoom avant / arrière / réinitialiser</p>
              <p>G / Shift+G</p><p className="text-right text-foreground">Haut / Bas</p>
            </div>
          </GlassPanel>
        </div>
      )}
      ...
      <Sheet open={settingsOpen} onOpenChange={setSettingsOpen}>
        <SheetContent side="bottom" className="bg-black text-white border-white/10 h-[82vh] overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Paramètres du lecteur</SheetTitle>
            <SheetDescription>Personnalisez la mise en page, les contrôles et le comportement des sources.</SheetDescription>
          </SheetHeader>

          <div className="mt-6 space-y-5 text-sm">
            <div>
              <label className="text-xs uppercase tracking-wider text-white/50">Source préférée</label>
              <select
              ...
              >
                <option value="auto">Auto (suivre la meilleure source)</option>
            ...
            <div>
              <label className="text-xs uppercase tracking-wider text-white/50">Langue préférée</label>
              <select
            ...
            <div>
              <label className="text-xs uppercase tracking-wider text-white/50">Mode de lecture</label>
              <select
                value={readerSettings.readingMode}
                onChange={(event) => updateReaderSettings({ readingMode: event.target.value as ReadingMode })}
                className="mt-1 w-full rounded-lg border border-white/15 bg-black/60 px-3 py-2"
              >
                <option value="long-strip">Bande continue</option>
                <option value="paged">Paginé</option>
              </select>
            </div>

            <div>
              <label className="text-xs uppercase tracking-wider text-white/50">Mode d'adaptation</label>
              <select
              ...
              >
                ...
                <option value="height">Adapter à la hauteur</option>
              </select>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span>Charger automatiquement le chapitre suivant</span>
                <Switch
