...
export default function MangaPage() {
      ...
      .map(([volume, chaptersInVolume]) => {
        ...
        return {
          ...
          firstChapterTitle:
            ...
            (firstReadable?.chapter.chapterNumber != null ? `Chapitre ${firstReadable.chapter.chapterNumber}` : ""),
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center p-6">
          ...
          <h1 className="text-2xl font-bold mb-2">Détails du manga indisponibles</h1>
          <p className="text-muted-foreground mb-4">Nous n'avons pas pu récupérer les informations pour cet ID de manga.</p>
        ...
        <main className={cn(
          ...
          <GlassPanel className="p-6 md:p-8 border border-amber-500/30">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-black">Avertissement de contenu sensible</h1>
                <p className="mt-2 text-muted-foreground">
                  Ce manga est marqué comme contenu mature. Continuez uniquement si vous êtes à l'aise avec du contenu 18+.
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4">
              <p className="text-sm text-muted-foreground">
                Titre : <span className="text-foreground font-semibold">{info.canonicalTitle || info.title?.english || info.title?.romaji || "Inconnu"}</span>
              </p>
            </div>
  ...
  return (
    <>
      <Helmet>
        <title>{displayTitle} - Lire le manga | AniStream</title>
        <meta name="description" content={info.synopsis?.slice(0, 160) || `Lire ${displayTitle} en ligne gratuitement.`} />
        <meta property="og:title" content={`${displayTitle} - Lire le manga | AniStream`} />
        <meta property="og:description" content={info.synopsis?.slice(0, 160) || `Lire ${displayTitle} en ligne gratuitement.`} />
      ...
      <div className="min-h-screen bg-background text-foreground overflow-x-hidden selection:bg-primary/30">
        ...
        <main className={cn(
            ...
            <div className="flex flex-col justify-end space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-4 pt-4">
                ...
                >
                  ...
                  {hasSavedProgress ? "Continuer la lecture" : "Lire depuis le chapitre 1"}
                </button>
                <AddToPlaylistButton
                ...
                >
                  ...
                  {!user ? "Connectez-vous pour la liste" : isInReadlist ? "Dans la liste" : "Ajouter à la liste"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-8">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-muted-foreground bg-muted px-3 py-1 rounded-full">
                        {isChapterListLoading
                          ? "Chargement..."
                          : hasChapterFilters
                          ? `${filteredDisplayChapters.length} sur ${displayChapters.length} correspondances`
                          : `${displayChapters.length} disponibles`}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="relative flex-1 min-w-[220px]">
                      ...
                      <input
                        ...
                        onChange={(event) => setChapterSearchTerm(event.target.value)}
                                                        placeholder="Rechercher chapitre n°/titre/fournisseur"
                  ...
                  {hasChapterGroups && (
                    <p className="text-xs text-muted-foreground">
                      {(() => {
                        ...
                        if (!activeRange) return `Affichage de ${filteredDisplayChapters.length} chapitres`;
                        return `Affichage des chapitres ${activeRange.start}-${activeRange.end} sur ${filteredDisplayChapters.length}`;
                      })()}
                    </p>
                  )}
                </div>
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-sm truncate group-hover:text-primary transition-colors">
                              {chapter.chapterTitle || `Chapitre ${chapter.chapterNumber ?? "?"}`}
                            </p>
                            ...
                            <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider mt-1">
                              {releaseDate ? new Date(releaseDate).toLocaleDateString() : "Date inconnue"}
                            </p>
                  ...
                  <GlassPanel className="p-8 text-center">
                    <p className="text-muted-foreground">
                      {sortedChapters.length === 0
                        ...
                        : hasChapterFilters
                        ? "Aucun chapitre ne correspond à vos filtres ou à votre recherche."
                        : "Aucun chapitre ne correspond aux filtres de fournisseur/langue sélectionnés. Essayez Auto."}
                    </p>
                  </GlassPanel>
                )}
              </section>
                      ...
                      <GlassPanel key={`volume-${volumeItem.volume}`} className="p-4 border border-white/10">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className="text-[10px] uppercase tracking-wider text-muted-foreground">
                            {volumeItem.readableCount}/{volumeItem.chapterCount} lisibles
                          </span>
                        </div>

                        <p className="mt-2 text-xs uppercase tracking-wider text-muted-foreground">
                          Chapitres {volumeItem.rangeLabel}
                        </p>

                        {volumeItem.firstChapterTitle && (
                          <p className="mt-1 text-xs text-foreground/80 line-clamp-1">
                            Début : {volumeItem.firstChapterTitle}
                          </p>
                        )}
                        ...
                        {volumeItem.readableChapters.length > 0 && (
                          <div className="mt-3 flex flex-wrap gap-1.5">
                            {volumeItem.readableChapters.slice(0, 6).map((chapterItem) => (
                              ...
                              >
                                {chapterItem.chapterNumber != null ? `Ch ${chapterItem.chapterNumber}` : "Ouvrir le chapitre"}
                              </button>
                            ))}
                            {volumeItem.readableChapters.length > 6 && (
                              <span className="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-[10px] uppercase tracking-wider text-muted-foreground">
                                +{volumeItem.readableChapters.length - 6} de plus
                              </span>
                            )}
                          </div>
                        )}
              ...
              <section>
                ...
                {loadingRecommendations ? (
                  ...
                  <GlassPanel className="p-5">
                    <p className="text-sm text-muted-foreground">
                      Les recommandations se préparent. Revenez dans un instant.
                    </p>
                  </GlassPanel>
                )}
              </section>
            ...
            <div className="space-y-4">
              <GlassPanel className="p-5">
                <h4 className="font-display font-bold mb-3">Métadonnées</h4>
                <div className="mb-4">
                  ...
                  >
                    <option value="auto">Auto (Meilleure source par chapitre)</option>
                  ...
                  >
                    <option value="auto">
                      {profilePreferredMangaLanguage === "auto"
                        ? "Auto (Toutes les langues)"
                        : `Auto (Paramètre : ${formatLanguageLabel(profilePreferredMangaLanguage)})`}
                    </option>
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  <p>Statut : <span className="text-foreground">{info.status || "inconnu"}</span></p>
                  <p>Format : <span className="text-foreground">{info.mediaType || "manga"}</span></p>
                  <p>Total chapitres : <span className="text-foreground">{info.totalChapters ?? "?"}</span></p>
                ...
                <div className="mt-4">
                  ...
                  {info.genres && info.genres.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground">Aucun genre disponible.</p>
                  )}
                </div>
              </GlassPanel>
            </div>
