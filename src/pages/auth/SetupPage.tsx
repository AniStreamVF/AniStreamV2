...
const TEXT_VARIATIONS = [
  {
    title: "Votre hub anime",
    desc: "Téléchargez, organisez et regardez vos séries d'anime préférées - tout dans une seule application."
  },
  {
    title: "Regardez partout",
    desc: "Téléchargez des épisodes pour les regarder hors ligne et ne manquez jamais un moment de vos séries préférées."
  },
  {
    title: "Votre collection",
    desc: "Créez votre bibliothèque d'anime personnelle avec des téléchargements faciles et une organisation transparente."
  },
];
...
export default function SetupPage() {
  ...
  const handleComplete = () => {
    localStorage.setItem('AniStream_setup_complete', 'true');
    if (isDesktop) {
      localStorage.setItem('AniStream_download_path', downloadPath);
      localStorage.setItem('AniStream_discord_rpc', String(discordEnabled));
    }
    navigate('/');
  };
        ...
        >
          ...
          <div className="mb-10">
            ...
            <h2 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">
              Bienvenue sur AniStream
            </h2>
            <p className="text-muted-foreground">
              Configurons votre expérience anime en quelques étapes.
            </p>
          </div>
              ...
              {step === 1 && (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-foreground">Emplacement de stockage</h3>
                      <p className="text-sm text-muted-foreground">
                        {isMobile ? "Vos téléchargements seront sauvegardés en toute sécurité" : "Où vos animes doivent-ils être sauvegardés ?"}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="font-medium text-foreground">Stockage de l'application</span>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          Les téléchargements seront sauvegardés dans le stockage privé de l'application.
                        </p>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        Conseil : les fichiers ne sont accessibles que via cette application pour des raisons de sécurité.
                      </p>
                    </div>
                  ) : (
                    // Desktop: Show directory picker
                    <div className="space-y-3">
                      <label className="text-sm font-medium text-foreground">Dossier de téléchargement</label>
                      <div className="flex items-center gap-2 p-2 rounded-xl bg-muted/30 border border-border">
                        <div className="flex-1 px-3 py-2 text-sm font-mono text-muted-foreground truncate">
                          {downloadPath || 'Sélectionnez un dossier...'}
                        </div>
                        <Button 
                          variant="secondary"
                          size="sm"
                      ...
                      <p className="text-xs text-muted-foreground">Environ 2 Go par série d'anime.</p>
                    </div>
                  )}
                  ...
                  >
                    {isMobile ? 'Commencer' : 'Continuer'} <ChevronRight className="w-4 h-4 ml-2" />
                  </Button>
                </motion.div>
              )}
              ...
              {step === 2 && (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-foreground">Confidentialité et intégration</h3>
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border hover:border-primary/30 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-foreground">Riche présence Discord</span>
                          <span className="text-xs text-muted-foreground">Affichez ce que vous regardez</span>
                        </div>
                      </div>
          ...
          <div className="mt-8 pt-6 border-t border-border/50">
            <p className="text-xs text-muted-foreground text-center">
              Vous pouvez modifier ces paramètres à tout moment dans les préférences de l'application.
            </p>
          </div>
        </motion.div>
      </div>
