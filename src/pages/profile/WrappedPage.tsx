      ...
      <main className={cn(
        ...
        >
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <div className="text-muted-foreground text-sm font-medium">Vos statistiques {currentYear}</div>
              <h1 className="font-display text-3xl md:text-4xl font-black tracking-tight gradient-text">
                {profile?.display_name || 'Votre'} bilan
              </h1>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <StatCard
            ...
            sublabel={`${stats.completedEpisodes} terminés`}
          />
          <StatCard
            icon={Film}
            iconClass="text-teal-400"
            label="Séries d'anime"
            value={stats.uniqueAnime.toLocaleString()}
            sublabel="titres uniques"
          />
          <StatCard
            icon={Flame}
            ...
            sublabel={streak.currentStreak > 0 ? `${streak.currentStreak}j en cours` : 'Continuez comme ça !'}
          />
        </motion.div>
            ...
            >
              <GlassPanel className="p-5 flex flex-col sm:flex-row items-center gap-5 border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
                ...
                <div className="flex-1 min-w-0 text-center sm:text-left">
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest mb-1">Votre rang actuel</p>
                  <h2 className={cn('text-2xl font-black font-display', rankTier.color)}>{rankTier.name}</h2>
                  <p className="text-sm text-muted-foreground mt-0.5">
                    {stats.totalEpisodes.toLocaleString()} épisodes regardés
                  </p>
                  {nextRank && (
                    <div className="mt-3 space-y-1">
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>Progression vers <span className={nextRank.tier.color}>{nextRank.tier.name}</span></span>
                        <span>{nextRank.needed} épisodes supplémentaires</span>
                      </div>
            ...
            >
              <GlassPanel className="p-5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 className="font-display font-bold text-base">Les plus regardés</h2>
                </div>
            ...
            >
              <GlassPanel className="p-5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 className="font-display font-bold text-base">Activité mensuelle</h2>
                </div>
          ...
          <GlassPanel className="p-5">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-display font-bold text-base">Succès</h2>
        ...
        {history.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
            ...
            <h2 className="font-display text-2xl font-black">Pas encore de statistiques</h2>
            <p className="text-muted-foreground max-w-xs">
              Commencez à regarder des animes pour construire vos statistiques et débloquer des succès !
            </p>
