...
export default function PublicPlaylistPage() {
  ...
  if (!playlist) return <div className="min-h-screen flex items-center justify-center">Playlist introuvable</div>;
      ...
      <div className={embed ? 'max-w-full' : 'max-w-4xl mx-auto'}>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Par {ownerName}</p>
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => { navigator.clipboard.writeText(shareUrl); toast.success('Lien copié'); }} className="gap-2">
              <Share2 className="w-4 h-4" />
              Copy Link
            </Button>
            {!embed && playlist.embed_allowed && (
              <Button variant="outline" onClick={() => { navigator.clipboard.writeText(embedCode); toast.success('Code intégré copié'); }} className="gap-2">
                Copy Embed
              </Button>
            )}
          </div>
        ...
        {playlist_items.length > 0 ? (
          ...
          <div className="text-center py-12 text-muted-foreground">Cette playlist est vide</div>
        )}
      </div>
    </div>
  );
