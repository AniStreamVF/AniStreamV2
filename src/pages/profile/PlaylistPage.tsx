...
const PLAYLIST_EDIT_DRAFT_STORAGE_PREFIX = 'AniStream_playlist_edit_draft_v1';
      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        <main className="relative z-10 pl-0 md:pl-20 lg:pl-24 w-full">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-20 text-center">
            ...
            <h1 className="text-2xl font-bold mb-2">Connectez-vous pour créer des playlists</h1>
            <p className="text-muted-foreground mb-6">Créez et gérez vos playlists d'anime et manga</p>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="De quoi parle cette playlist ?"
                      rows={3}
                    />
                  </div>
          ...
          {isLoading ? (
            ...
            <GlassPanel className="p-12 text-center">
              ...
              <h2 className="text-xl font-bold mb-2">Aucune playlist</h2>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cela supprimera définitivement cette playlist et la retirera de tous les collaborateurs. Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function PlaylistViewPage() {
  ...
  const openEdit = () => {
    if (!playlist) return;
    if (!canEdit) {
      toast.error('Vous n\'avez pas les droits de modification sur cette playlist');
      return;
    }
  ...
  const handleGenerateSlug = async () => {
    ...
    toast.error('Échec de la génération d\'un lien de partage unique. Réessayez plus tard.');
  };
  ...
  const handleRemoveItem = async (animeId: string) => {
    if (!playlist) return;
    if (!canEdit) {
      toast.error('Vous n\'avez pas la permission de modifier cette playlist');
      return;
    }
  ...
  const handleLaunchWatchRoom = (animeId: string, animeName: string, animePoster?: string | null) => {
    ...
    if (media.kind !== 'anime' || !media.id) {
      toast.error('Les salons de visionnage ne supportent que les animes pour le moment.');
      return;
    }
          ...
          <GlassPanel className="p-6">
            ...
            {loadingItems ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">Cette playlist est vide</p>
                {canEdit && (
                  <p className="text-sm text-muted-foreground mt-2">
                    Ajoutez un anime ou manga depuis la page de détail via le bouton Ajouter à la playlist
                  </p>
                )}
              </div>
            )}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mt-6">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-muted/40 border border-border/60">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold truncate">Propriétaire</p>
                  ...
                  {collaboratorSearch.trim().length >= 2 && (
                    <div className="max-h-44 overflow-y-auto rounded-lg border border-border/60 bg-muted/30 divide-y divide-border/50">
                      {isSearchingCollaborators ? (
                        <p className="text-sm text-muted-foreground p-3">Recherche d'utilisateurs...</p>
                      ) : collaboratorSearchResults.length === 0 ? (
                        <p className="text-sm text-muted-foreground p-3">Aucun utilisateur trouvé.</p>
                      ) : (
            ...
            <GlassPanel className="p-6 xl:col-span-2">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold">Discussion de la playlist</h2>
              ...
              <div className="space-y-4">
                {topLevelComments.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Aucun commentaire. Lancez la conversation.</p>
                ) : (
      ...
      <Dialog open={showEdit} onOpenChange={setShowEdit}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label>Description de partage (optionnelle)</Label>
              <Textarea
                value={editShareDesc}
                onChange={(e) => setEditShareDesc(e.target.value)}
                rows={2}
                placeholder="Un court texte affiché sur les pages publiques"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <Input value={editShareSlug || ''} readOnly placeholder="Pas encore de lien de partage" />
              <Button variant="outline" onClick={handleGenerateSlug} className="gap-2">
                <GripVertical className="w-4 h-4" />
                Generate
              </Button>
              {editShareSlug && (
                <Button variant="outline" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/p/${editShareSlug}`); toast.success('Lien de partage copié'); }} className="gap-2">
                  <Share2 className="w-4 h-4" />
                  Copy Link
                </Button>
              )}
      ...
      <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Supprimer la playlist ?</AlertDialogTitle>
            <AlertDialogDescription>
              Cela supprimera définitivement "{playlist.name}" et tous ses éléments. Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Annuler</AlertDialogCancel>
