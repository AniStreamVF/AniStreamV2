...
const TABS: { id: TabType; label: string; icon: React.ReactNode }[] = [
  { id: 'for-you', label: 'Pour vous', icon: <Sparkles className="w-4 h-4" /> },
  { id: 'ai-recs', label: 'Recommandations IA', icon: <Brain className="w-4 h-4" /> },
  { id: 'all', label: 'Tout', icon: <BookOpen className="w-4 h-4" /> },
  { id: 'watching', label: 'En cours', icon: <TrendingUp className="w-4 h-4" /> },
  { id: 'completed', label: 'Terminé', icon: <CheckCircle className="w-4 h-4" /> },
  { id: 'plan', label: 'Prévu', icon: <Clock className="w-4 h-4" /> },
  { id: 'on-hold', label: 'En pause', icon: <Pause className="w-4 h-4" /> },
  { id: 'dropped', label: 'Abandonné', icon: <X className="w-4 h-4" /> },
];
...
export default function FavoritesPage() {
  ...
  const toAniListCard = (media: AniListMedia) => {
    const animeId = `anilist-${media.id}`;
        ...
        <main className={cn(
          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
            ...
            >
              ...
              <h1 className="text-3xl font-black mb-4">Vos favoris</h1>
              <p className="text-muted-foreground mb-8 max-w-sm mx-auto">
                Connectez-vous pour voir vos recommandations personnalisées et gérer votre liste de suivi
              </p>
              <button
        ...
        <div className="mb-8 md:mb-12">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl md:text-5xl font-black tracking-tighter">Favoris</h1>
                <p className="text-muted-foreground font-medium">
                  {showRecommendations
                    ? 'Sélectionné pour vous'
                    : `${filteredWatchlist.length} anime sauvegardés dans votre collection`}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {selectMode ? 'Terminé' : 'Gérer la liste'}
              </button>
        ...
        {recommendations && recommendations.length > 0 && activeTab !== 'for-you' && (
          <div className="mb-8 md:mb-12">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-xs text-muted-foreground ml-2">(Basé sur vos préférences)</span>
            </div>
              ...
              <>
                ...
                {showRecommendations && smartFavorites && smartFavorites.length > 0 && (
                  <div className="mb-12">
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <h2 className="text-xl font-bold">Favoris suggérés</h2>
                      <span className="text-xs text-muted-foreground ml-2">(Basé sur votre historique)</span>
                    </div>
                ...
                {showRecommendations && (
                  <div className="mb-12">
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <h2 className="text-xl font-bold">Favoris populaires</h2>
                      <span className="text-xs text-muted-foreground ml-2">(Converti en ID AniStream)</span>
                    </div>

                    {loadingAniListFavorites ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                        {Array.from({ length: 6 }).map((_, idx) => (
                          <Skeleton key={`anilist-fav-skeleton-${idx}`} className="aspect-[2/3] rounded-xl" />
                        ))}
                      </div>
                    ) : aniListFavorites.length === 0 ? (
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-muted-foreground">
                        Les favoris AniList sont temporairement indisponibles. Réessayez bientôt.
                      </div>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                        {aniListFavorites.slice(0, 12).map((media) => {
                          const mapped = toAniListCard(media);
                          return (
                            <div key={`anilist-favorite-${media.id}`} className="relative group">
                              <Link to={`/anime/${mapped.animeId}`} className="block aspect-[2/3] rounded-xl overflow-hidden mb-2 relative">
                                <img src={getProxiedImageUrl(mapped.animePoster)} className="w-full h-full object-cover" alt={mapped.animeName} loading="lazy" />
                                  ...
                                  <button
                                    onClick={(e) => {
                                      ...
                                      addToWatchlist.mutate({
                                        ...
                                        animeName: mapped.animeName,
                                        animePoster: mapped.animePoster || undefined,
                                        status: 'plan_to_watch'
                                      });
                                    }}
                                    className="p-2 rounded-full bg-primary text-white hover:scale-110 transition-transform"
                                    title="Ajouter à la liste de suivi"
                                  >
                                    <Heart className="w-4 h-4 fill-current" />
                                  </button>
                                </div>
                              </Link>
                              <h3 className="text-xs font-bold truncate" title={mapped.animeName}>{mapped.animeName}</h3>
                              <p className="text-[10px] text-muted-foreground truncate">
                                Fav. : {mapped.favourites ?? 'N/D'} • Score : {mapped.score ? `${mapped.score}%` : 'N/D'}
                              </p>
                            </div>
                          );
                        })}
                      ...
                      <div className="relative p-8 md:p-12 rounded-[2.5rem] border border-white/10 flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden">
                        ...
                        <div className="flex-1 space-y-4 text-center md:text-left z-10">
                          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10 backdrop-blur-md">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-white/80">Fonctionnalité IA Premium</span>
                          </div>
                          <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-white">
                            ANALYSE PROFONDE DE VOTRE<br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-white/40">PROFIL DE GOÛTS ANIME</span>
                          </h2>
                          <p className="text-muted-foreground font-medium max-w-md">
                            Notre moteur de machine learning a analysé vos habitudes de visionnage. Découvrez pourquoi vous allez adorer ces titres.
                          </p>
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-6">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">Analyse IA de vos préférences</p>
                          </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                          <div>
                            <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-4">Genres préférés</h3>
                          ...
                          <div>
                            <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-4">Style de visionnage</h3>
                            <div className="space-y-3">
                              <div className="flex items-center justify-between text-sm">
                                <span className="font-medium">Score de diversité</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-10 md:gap-x-6 md:gap-y-12">
                  {(showMLRecs ? (mlRecommendations || []) : (showRecommendations ? (recommendations || []) : filteredWatchlist)).map((item: any, index: number) => {
                    ...
                    const recommendationReason = item.reason || (Array.isArray(item.reasons) ? item.reasons[0] : undefined);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={cn("transition-transform duration-500", selectMode && selectedIds.includes(anim.id) && "scale-95")}>
                          ...
                          {!showRecommendations && !showMLRecs && item.status && (
                            <div className="mt-4 px-1">
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] font-black uppercase tracking-tighter text-muted-foreground opacity-60">Statut</span>
                          ...
                          {(showRecommendations || showMLRecs) && recommendationReason && (
                            <div className="mt-3 px-2 line-clamp-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <span className="text-[10px] font-bold text-pink-400 uppercase tracking-widest">{recommendationReason}</span>
                            </div>
                          )}
                ...
                {(!isLoading && (
                  showMLRecs
                    ...
                    : (showRecommendations ? !recommendations?.length : !filteredWatchlist.length)
                )) && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                    ...
                    <h3 className="text-xl font-bold mb-2">Aucun anime trouvé</h3>
                    <p className="text-muted-foreground max-w-xs mx-auto">
                      Explorez plus de titres pour remplir cette collection avec vos favoris.
                    </p>
                  </motion.div>
                )}
              </>
