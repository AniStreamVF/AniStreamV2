      ...
      <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
        ...
        <main className="relative z-10 pl-6 md:pl-32 pr-6 py-6 max-w-[1400px] mx-auto pb-24 md:pb-6">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Classez vos animes préférés</p>
          </div>
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <main className="relative z-10 pl-6 md:pl-32 pr-6 py-6 max-w-[1400px] mx-auto pb-24 md:pb-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-display text-3xl md:text-4xl font-bold mb-2">Classements</h1>
            <p className="text-muted-foreground">Classez et partagez vos préférences d'anime</p>
          </div>
          {user && (
            ...
            <TabsContent value="my-lists">
              {loadingUser ? (
                ...
                <div className="text-center py-12">
                  <p className="text-muted-foreground mb-4">Vous n'avez pas encore créé de classement</p>
                  <Button onClick={handleCreate} className="gap-2">
                    <Plus className="w-4 h-4" />
          ...
          <TabsContent value="community">
            {loadingPublic ? (
              ...
              <TierListGrid
                ...
                emptyMessage="Aucun classement public pour le moment. Soyez le premier à en créer un !"
              />
            )}
          </TabsContent>
        </Tabs>
...
export function TierListViewPage() {
  ...
  const handleDelete = async () => {
    if (!tierList || !confirm('Êtes-vous sûr de vouloir supprimer ce classement ?')) return;

    try {
      await deleteMutation.mutateAsync(tierList.id);
      toast.success('Classement supprimé');
      navigate('/tierlists');
    } catch {
      toast.error('Échec de la suppression du classement');
    }
  };
  ...
  const handleWatchWithFriends = (animeId: string, animeTitle: string, animeImage: string) => {
    if (animeId.startsWith('char-')) {
      toast.info('Les salons de visionnage ne peuvent être lancés que pour des animes');
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <p className="text-muted-foreground mb-4">Classement introuvable ou privé</p>
          <Button onClick={() => navigate('/tierlists')}>Parcourir les classements</Button>
        </div>
      </div>
    );
  }
        ...
        {(canManageCollaborators || collaborators.length > 0) && (
          <GlassPanel className="p-6 mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-semibold text-lg">Collaborateurs</h2>
            </div>
