...
export default function AdminPage() {
  ...
  const toggleMaintenance = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(maintenanceMode?.is_active ? 'Mode maintenance désactivé' : 'Mode maintenance activé');
    },
    onError: () => {
      toast.error('Échec du changement du mode maintenance');
    },
  });
  ...
  const toggleAdmin = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['admin_users'] });
      toast.success(variables.makeAdmin ? 'Utilisateur promu administrateur' : 'Privilèges d\'administrateur révoqués');
    },
    onError: () => {
      toast.error('Échec de la mise à jour du statut administrateur');
    },
  });
  ...
  const sendMessage = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error?.code === '42P01') {
        toast.error('Table des messages admin introuvable. Veuillez exécuter la migration.');
      } else if (error?.message?.includes('permission denied') || error?.code === '42501') {
        toast.error('Permission refusée. Assurez-vous d\'être administrateur.');
      } else {
        toast.error('Échec de l\'envoi du message : ' + (error?.message || 'Erreur inconnue'));
      }
    },
  });
  ...
  const toggleModerator = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['admin_users'] });
      toast.success(variables.makeModerator ? 'Utilisateur promu modérateur' : 'Privilèges de modérateur révoqués');
    },
    onError: () => {
      toast.error('Échec de la mise à jour du statut modérateur');
    },
  });
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Gérer les utilisateurs et le contenu</p>
              </div>
            </div>
          </div>
              ...
              <SheetTrigger asChild>
                <Button variant="outline" className="w-full justify-between h-14 px-6 bg-card/40 border-white/5 rounded-2xl">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold">Menu du tableau de bord</span>
                  </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-border/50">
                          ...
                          <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">Nom d'utilisateur</th>
                                ...
                                {isAdmin && (
                                  <>
                                    ...
                                    <Button
                                      ...
                                      title={user.role === 'moderator' ? "Retirer le modérateur" : "Nommer modérateur"}
                                    >
                                    ...
                                    <Button
                                      ...
                                      title={user.is_admin ? "Retirer l'administrateur" : "Nommer administrateur"}
                                    >
            ...
            <TabsContent value="notifications">
              <div className="grid grid-cols-1 gap-8">
                <div className="space-y-6">
                  <GlassPanel className="p-6">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input
                          placeholder="Maintenance, Mise à jour, etc."
                          value={messageTitle}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-sm font-medium mb-1.5 block">Type de cible</label>
                          <select
                          ...
                          >
                            <option value="all">Diffusion (Tous les utilisateurs)</option>
                        ...
                        {selectedUserId === 'search' && (
                          <div>
                            ...
                            <Input placeholder="Entrez l'ID..." className="bg-muted/30" />
                          </div>
                        )}
                      </div>
                  ...
                  <GlassPanel className="p-6">
                    <h3 className="font-medium mb-4">Historique des notifications envoyées</h3>
                    {loadingSentMessages ? (
                          ...
                          <div key={msg.id} className="p-3 rounded-lg bg-muted/20 border border-border/30 text-xs">
                            <div className="flex justify-between items-start mb-1">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() => {
                                  if (confirm('Supprimer cette notification ?')) deleteMessage.mutate(msg.id);
                                }}
                ...
                <div className="space-y-4">
                  ...
                  Annuler
                    ...
                    <Button
                      ...
                      disabled={banUser.isPending}
                    >
                      {banUser.isPending ? 'Bannissement...' : 'Bannir l\'utilisateur'}
                    </Button>
                  </div>
                </div>
              </GlassPanel>
        ...
        <SheetContent side="right" className="w-[400px] sm:w-[500px] p-6 border-white/5 bg-background/95 backdrop-blur-xl border-l">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground uppercase tracking-widest text-[10px]">Sujet</label>
              <Input
            ...
            >
              ...
              {sendMessage.isPending ? 'Envoi...' : 'Envoyer maintenant'}
            </Button>
          </div>
        </SheetContent>
      </Sheet >
          ...
          <GlassPanel className="p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  placeholder="Entrez le titre de diffusion..."
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (!users || users.length === 0) {
                        toast.error('Aucun utilisateur trouvé à notifier');
                        return;
                      }
                      ...
                      toast.success(`Notification diffusée à ${users.length} utilisateur(s) !`);
                      setShowBroadcastModal(false);
                      setBroadcastTitle('');
                      setBroadcastMessage('');
                    } catch (error) {
                      console.error('Error sending broadcast notification:', error);
                      toast.error('Échec de l\'envoi de la notification diffusée');
                    }
                  }}
