...
import { AniStream_API_URL, withClientHeaders } from '@/lib/api/api-client';
...
const DEFAULT_BACKEND_HEALTH_URL = 'https://api.AniStream.me/health';
...
const KNOWN_PROXY_NODES: KnownProxyNode[] = [
  {
    id: 'proxy-node-hoko',
    url: 'https://hoko.AniStream.me/api/v1/streamingProxy',
    type: 'nodejs',
  },
];
...
export default function StatusPage() {
  ...
  const [services, setServices] = useState<ServiceStatus[]>([
    { name: 'Site web AniStream', status: 'checking', icon: <Globe className="w-5 h-5" />, description: 'Frontend du site principal', url: window.location.origin },
    { name: 'Backend AniStream', status: 'checking', icon: <Zap className="w-5 h-5" />, description: 'API Hono unifiée', url: DEFAULT_BACKEND_HEALTH_URL },
    { name: 'API Supabase', status: 'checking', icon: <Database className="w-5 h-5" />, description: 'Base de données et authentification' },
    { name: 'API Jikan', status: 'checking', icon: <Server className="w-5 h-5" />, description: 'Fournisseur de métadonnées MyAnimeList', url: 'https://api.jikan.moe/v4/health' },
    { name: 'Images', status: 'checking', icon: <Image className="w-5 h-5" />, description: 'Affiches et miniatures d\'anime', url: 'https://api.waifu.pics/sfw/waifu' },
    { name: 'Infrastructure de streaming', status: 'checking', icon: <Play className="w-5 h-5" />, description: 'Services de livraison vidéo' },
  ]);
  const [lastChecked, setLastChecked] = useState<Date>(new Date());
  const [isRefreshing, setIsRefreshing] = useState(false);
  const proxyStatusEndpoint = useMemo(
    () => resolveProxyStatusEndpoint(AniStream_API_URL, import.meta.env.VITE_BACKEND_ORIGIN),
    []
  );
  const backendHealthEndpoint = useMemo(
    () => resolveBackendHealthEndpoint(AniStream_API_URL, import.meta.env.VITE_BACKEND_ORIGIN),
    []
  );
  ...
  const classifyProxyType = (url: string) => {
    ...
    if (lower.includes('workers.dev') || lower.includes('cloudflare') || lower.includes('kira.AniStream.me')) return 'cf';
    if (lower.includes('hoko.AniStream.me')) return 'nodejs';
    if (lower.includes('bun')) return 'bun';
    return 'nodejs';
  };

  const loadScraperHealth = useCallback(async () => {
    const response = await fetch(`${AniStream_API_URL}/health/scrapers`, {
  ...
  const checkServices = async () => {
    ...
    try {
      ...
      // Check AniStream Website
      await checkService('Site web AniStream', async () => {
      ...
      // Check AniStream Backend
      await checkService('Backend AniStream', async () => {
        const start = Date.now();
        try {
      ...
      // Check Supabase
      await checkService('API Supabase', async () => {
        const start = Date.now();
      ...
      // Check Jikan API
      await checkService('API Jikan', async () => {
        const start = Date.now();
        try {
      ...
      // Check Image Assets
      await checkService('Images', async () => {
        const start = Date.now();
        try {
      ...
      await checkService('Infrastructure de streaming', async () => {
        const start = Date.now();
        try {
  ...
  const overallStatus = hasServiceDown || hasProxyDown || hasScraperDown
    ? 'Défaillance critique du système'
    : hasServiceDegraded || hasProxyDegraded || hasScraperDegraded
      ? 'Dégradation partielle du système'
  ...
  const dashboardCards = [
    {
      ...
      detail: `${services.filter((service) => service.status === 'operational').length}/${services.length} services en ligne`,
    },
    {
      label: 'Nœuds proxy',
      value: `${proxies.filter((proxy) => proxy.status === 'online').length}/${proxies.length}`,
      detail: `${proxies.filter((proxy) => proxy.status === 'degraded').length} dégradés`,
    },
    {
      label: 'Vérifications des scraper',
      value: `${scraperSummary.operational}/${scraperSummary.total}`,
      detail: `${scraperSummary.degraded} dégradés, ${scraperSummary.down} hors ligne`,
    },
    {
      label: 'MTTR des incidents',
      value: mttrMinutes !== null ? `${mttrMinutes}m` : '--',
      detail: `${incidents.filter((incident: any) => incident.is_active).length} incidents actifs`,
    },
  ];
      ...
      <main className="relative z-10 pl-6 md:pl-32 pr-6 py-6 max-w-[1400px] mx-auto pb-24 md:pb-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          >
            ...
            <span className="text-sm font-medium text-muted-foreground group-hover:text-foreground">Retour</span>
          </button>
        </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1 flex items-center gap-2">
                    ...
                    Tableau de bord de l'infrastructure réseau AniStream
                  </p>
                </div>
              </div>
              <Button
              ...
              <GlassPanel className={`p-6 border-b-2 hover:translate-y-[-4px] transition-all duration-300 ${getStatusOutlineColor(service.status)}`}>
                ...
                <div className="mt-6 flex items-center justify-between">
                   <div className="flex flex-col">
                      <span className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Réponse</span>
                      <div className="flex items-center gap-2">
                         ...
                         <span className="text-sm font-medium">{service.status === 'operational' ? 'opérationnel' : service.status === 'degraded' ? 'dégradé' : 'hors ligne'}</span>
                      </div>
                   </div>
                   {service.latency !== undefined && (
                     <div className="flex flex-col items-end">
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Latence</span>
...
Santé des scraper
            ...
            <div className="text-xs text-muted-foreground font-mono bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              {scraperSummary.operational}/{scraperSummary.total} opérationnels
            </div>
          </div>
              ...
              >
                <GlassPanel className="p-5 border border-white/10 bg-white/5 backdrop-blur-xl">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Source générique</p>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium">{scraper.status === 'operational' ? 'opérationnel' : scraper.status === 'degraded' ? 'dégradé' : 'hors ligne'}</span>
            ...
            {scrapers.length === 0 && (
              <GlassPanel className="p-5 border border-dashed border-white/10 bg-white/5 backdrop-blur-xl sm:col-span-2 xl:col-span-3">
                <div className="flex items-center justify-center py-8 text-center text-muted-foreground">
                  ...
                  Les vérifications des scraper sont encore en cours.
                </div>
              </GlassPanel>
            )}
          </div>
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead>
                  <tr className="bg-white/5 border-b border-white/10">
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-[10px] text-muted-foreground">Identifiant du nœud</th>
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-[10px] text-muted-foreground">Type de fournisseur</th>
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-[10px] text-muted-foreground">Statut de déploiement</th>
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-[10px] text-muted-foreground">Latence réseau</th>
                    <th className="px-6 py-5 font-bold uppercase tracking-wider text-[10px] text-muted-foreground">Poids du payload</th>
                  </tr>
                </thead>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`font-bold text-xs uppercase tracking-widest 
                              ...
                              {proxy.status === 'online' ? 'en ligne' : proxy.status === 'degraded' ? 'dégradé' : 'hors ligne'}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">
                        <div className="flex flex-col items-center justify-center opacity-40">
                          ...
                          <p className="text-sm font-medium tracking-widest uppercase">Initialisation du pool de nœuds...</p>
                        </div>
                      </td>
                    </tr>
                  )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           <div className="lg:col-span-2">
              <div className="flex items-center gap-3 mb-6">
                 ...
                 <h2 className="font-display text-2xl font-bold">Maintenance et incidents</h2>
              </div>
              ...
              <GlassPanel className="p-6 border border-white/10 rounded-3xl min-h-[300px]">
                {loadingIncidents ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-lg font-medium">Tous les systèmes sont normaux</p>
                    <p className="text-sm text-muted-foreground mt-2 max-w-xs mx-auto">
                       Aucun incident actif ou maintenance programmée signalé au cours des 30 derniers jours. Conditions parfaites détectées.
                    </p>
                  </div>
                ) : (
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-6 pt-4 border-t border-white/5 text-[10px] text-muted-foreground font-mono flex items-center gap-2">
                              <span className={incident.is_active ? 'text-orange-500' : 'text-emerald-500'}>
                                ● {incident.is_active ? 'Début de l\'impact' : 'Résolution complète atteinte'}
                              </span>
           ...
           <div className="space-y-8">
              <div className="flex items-center gap-3 mb-6">
                 ...
                 <h2 className="font-display text-2xl font-bold">Résumé du réseau</h2>
              </div>
              
              <GlassPanel className="p-6 border border-white/10 rounded-3xl bg-primary/5">
                 <div className="space-y-6">
                    <div>
                       <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-3">Indice de santé global</p>
                       <div className="flex items-center gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       ...
                       <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                          <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Fréquence de vérification</p>
              ...
              <GlassPanel className="p-6 border border-white/10 rounded-3xl bg-secondary/5">
                 ...
                 <div className="flex justify-center flex-col items-center py-4 bg-black/20 rounded-2xl border border-white/5">
                    ...
                    <div className="text-[10px] text-muted-foreground mt-2 uppercase tracking-[0.2em] font-bold">Dernière vérification du système</div>
                 </div>
              </GlassPanel>
           </div>
        </div>
