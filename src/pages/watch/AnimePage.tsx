...
export default function AnimePage() {
  ...
  const { data: externalIds, isLoading: loadingExternalIds } = useExternalIds(contentAnimeId);
  ...
  const handleWatchNow = () => {
    if (episodesData?.episodes[0]) {
      navigate(`/watch/${encodeURIComponent(episodesData.episodes[0].episodeId)}?anime=${encodeURIComponent(contentAnimeId)}`);
    }
  };

  const handleEpisodeClick = (episodeId: string) => {
    navigate(`/watch/${encodeURIComponent(episodeId)}?anime=${encodeURIComponent(contentAnimeId)}`);
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-2">Détails de l'anime indisponibles</h1>
          <p className="text-muted-foreground mb-4">Nous n'avons pas pu récupérer les informations pour cet ID d'anime.</p>
  ...
  return (
    <>
      {/* SEO Meta Tags */}
      <Helmet>
        <title>{info.name} - Regarder en ligne | AniStream</title>
        <meta name="description" content={info.description?.slice(0, 160) || `Regarder ${info.name} en ligne gratuitement avec sous-titres.`} />
        <meta property="og:title" content={`${info.name} - Regarder en ligne | AniStream`} />
        <meta property="og:description" content={info.description?.slice(0, 160) || `Regarder ${info.name} en ligne.`} />
        ...
        <meta name="twitter:title" content={`${info.name} - Regarder en ligne`} />
        <meta name="twitter:description" content={info.description?.slice(0, 100) || `Regarder ${info.name}`} />
        ...
        <VideoBackground animeId={contentAnimeId!} poster={info.poster}>
          <main className={cn(
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16 pt-12 md:pt-24">
              {/* Poster */}
              <GlassPanel className="overflow-hidden aspect-[3/4]">
                <img
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {moreInfo.aired && (
                    <div>
                      <span className="text-xs text-muted-foreground uppercase tracking-wider">Diffusé</span>
                  ...
                  {moreInfo.status && (
                    <div>
                      <span className="text-xs text-muted-foreground uppercase tracking-wider">Statut</span>
                  ...
                  {producerNames.length > 0 && (
                    <div className="col-span-2 md:col-span-3">
                      <span className="text-xs text-muted-foreground uppercase tracking-wider">Producteurs</span>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-4">
                  <button
                    ...
                    className="h-12 sm:h-14 px-6 sm:px-8 rounded-full bg-primary text-primary-foreground font-bold text-base sm:text-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-2 glow-primary disabled:opacity-50"
                  >
        ...
        <main className="relative z-10 pl-6 md:pl-32 pr-6 max-w-[1800px] mx-auto pb-24 md:pb-6">
          ...
          <section className="mb-16">
            <div className="flex flex-col gap-4 mb-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="font-display text-2xl font-semibold">Épisodes</h2>
              ...
              {hasEpisodeGroups && (
                <p className="text-xs text-muted-foreground">
                  Affichage des épisodes {activeEpisodeGroup * EPISODES_PER_GROUP + 1}-{Math.min((activeEpisodeGroup + 1) * EPISODES_PER_GROUP, totalEpisodes)} sur {totalEpisodes}
                </p>
              )}
            </div>
            {loadingEpisodes ? (
          ...
          {producerAnimes.length > 0 && (
            <AnimeGrid
              ...
              title={`Plus de ${producerAnimeData?.producerName || producerNames[0] || 'ce producteur'}`}
            />
          )}

          {/* Related Animes */}
          {relatedAnimes.length > 0 && (
            <AnimeGrid animes={relatedAnimes.slice(0, 6)} title="Animes liés" />
          )}

          {/* Recommended */}
          {recommendedAnimes.length > 0 && (
            <AnimeGrid animes={recommendedAnimes.slice(0, 6)} title="Recommandé" />
          )}
        </main>
