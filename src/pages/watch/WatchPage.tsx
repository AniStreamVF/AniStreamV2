...
import { fetchEpisodeServers, fetchStreamingSources, fetchAniStreamEpisodeSources, trackEvent } from "@/lib/api";
import {
  buildDubSubtitles,
...
export default function WatchPage() {
  ...
  // Extract anime ID from query param, fall back to slug from episodeId
  const animeId = useMemo(() => {
    const params = new URLSearchParams(location.search);
    const fromParam = params.get('anime');
    if (fromParam) return fromParam;
    if (!decodedEpisodeId) return "";
    return decodedEpisodeId.split("?")[0];
  }, [decodedEpisodeId, location.search]);
  ...
  useEffect(() => {
    ...
    const shouldFallback = window.confirm("Aucune source VF disponible pour cet épisode. Passer en VOSTFR ?");
    if (shouldFallback) {
      setCategory("sub");
      selectRegularServer(-1);
      setSelectedProviderServerKey(null);
  ...
  // Detect seasons from episode number resets
  const seasons = useMemo(() => {
    const list = isOfflineMode ? offlineManifest?.episodes : episodesData?.episodes;
    if (!list || list.length === 0) return [];
    const result: { season: number; episodes: any[] }[] = [];
    let current: any[] = [list[0]];
    for (let i = 1; i < list.length; i++) {
      if (list[i].number <= list[i-1].number) {
        result.push({ season: result.length + 1, episodes: current });
        current = [];
      }
      current.push(list[i]);
    }
    if (current.length > 0) result.push({ season: result.length + 1, episodes: current });
    return result;
  }, [isOfflineMode, offlineManifest, episodesData]);

  // Find current episode using the sorted full list
  ...
  // Determine season from current episode index
  const currentSeasonIndex = useMemo(() => {
    let idx = 0;
    for (let s = 0; s < seasons.length; s++) {
      if (currentEpisodeIndex >= idx && currentEpisodeIndex < idx + seasons[s].episodes.length) {
        return s;
      }
      idx += seasons[s].episodes.length;
    }
    return Math.max(0, seasons.length - 1);
  }, [seasons, currentEpisodeIndex]);

  const [selectedSeason, setSelectedSeason] = useState(0);

  // Sync selectedSeason to current episode's season when it changes
  useEffect(() => {
    setSelectedSeason(currentSeasonIndex);
  }, [currentSeasonIndex]);
  ...
  const { data: fastStartData, isLoading: loadingFastStart } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        try {
          return await fetchAniStreamEpisodeSources(decodedEpisodeId, currentServer?.serverName || "hd-1", category);
        } catch {
          return empty;
        }
      }
  ...
  const { data: subSourcesData } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        try {
          return await fetchAniStreamEpisodeSources(decodedEpisodeId, combinedSourceServer, "sub");
        } catch {
          return empty;
        }
      }
  ...
  const resolvedSelectedSource = useMemo(() => {
    ...
    // Handle AniStream provider groups (index -5)
    if (selectedServerIndex === -5 && selectedProviderServerKey) {
    ...
    const runPreflight = async () => {
      ...
      if (!result.ok) {
        ...
        if (isLikelyHls) {
          ...
          setSourceReady(true);
          setSourcePreflightError("Échec de la préparation du flux. Tentative de repli HLS...");
          return;
        }

        setSourceReady(false);
        setSourcePreflightError("Échec de la préparation du flux. Nouvelle tentative du serveur...");
      } else {
        setSourceReady(true);
        setSourcePreflightError(null);
      }
  ...
  const providerSourceCount = useMemo(() => {
    return visibleSources.filter((source) =>
      ...
      !!source.providerName &&
      source.providerName !== 'AniStreamAPI' &&
  ...
  const languageGroups = useMemo<Record<string, any[]>>(() => {
    ...
    return visibleSources.reduce((acc: Record<string, any[]>, source) => {
      ...
      if (!source.providerName || source.providerName === 'AniStreamAPI' || source.langCode?.startsWith('marketplace-')) return acc;

      let lang = source.language || "Inconnu";
  ...
  const isVerifiedProvider = (providerKey?: string, serverName?: string) => {
    ...
    // Strictly limited to official HiAnime/AniStream servers only
    const verified = ['hianime', 'AniStreamapi'];
  ...
  const activeServerDisplayName = useMemo(() => {
    if (isOfflineMode) return "Hors ligne";
    if (selectedSource) {
  ...
  const requestSourceRefresh = useCallback((reason: string, options?: { purgeProviderFallback?: boolean }) => {
    ...
    setSourcePreflightError(`Actualisation des liens (${reason})...`);
  ...
  const handleEpisodeChange = (epId: string) => {
    ...
    if (isOfflineMode) {
      ...
      navigate(`/watch/${encodeURIComponent(epId)}?anime=${encodeURIComponent(animeId)}`);
    }
  };
              ...
              <div className="flex items-center gap-4">
                {viewCount !== undefined && viewCount > 0 && (
                  <span className="text-xs text-muted-foreground flex items-center justify-end gap-1">
                    ...
                    {formatViewCount(viewCount)} vues
                  </span>
                )}
              </div>
            </div>
                  ...
                  sources={
                    isOfflineMode
                      ...
                      : (activePlaybackSource && !activePlaybackSource.isEmbed
                        ...
                        : (sourceDataForPlayback?.sources || []).filter((s: any) => !s.isEmbed))
                  }
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                <h1 className="font-display text-lg md:text-2xl font-bold">
                  Épisode {currentEpisode?.number || serversData?.episodeNo || (isOfflineMode ? offlineManifest?.episodes.find((e: any) => e.id === decodedEpisodeId)?.number : "?")}
                </h1>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                {/* Secondary Actions */}
                <div className="flex items-center gap-1 mr-2 px-2 border-r border-border/30">
                  <button
            ...
            {isOfflineMode && (
              <GlassPanel className="p-4 md:p-5">
                <div className="text-sm text-muted-foreground text-center py-4">
                  Mode hors ligne — aucune sélection de serveur disponible
                </div>
              </GlassPanel>
            )}
          </div>
            ...
            <GlassPanel className="p-4 md:p-5 max-h-[400px] xl:max-h-[700px] flex flex-col">
              <div className="flex items-center gap-3 mb-3 md:mb-4">
                <ListVideo className="w-5 h-5 text-primary" />
                      <h3 className="font-display text-base md:text-lg font-semibold">Épisodes</h3>
                ...
                <div className="flex items-center gap-1.5 mb-3 overflow-x-auto">
                  {seasons.map((s, i) => (
                    <button
                      ...
                      onClick={() => {
                        setSelectedSeason(i);
                        const el = document.getElementById(`season-${s.season}`);
                        el?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                      }}
                      className={`px-2.5 py-1 text-xs rounded-lg font-medium whitespace-nowrap transition-colors ${selectedSeason === i
                        ? 'bg-primary text-primary-foreground'
              ...
              <div id="episode-list-container" className="flex-1 overflow-y-auto space-y-1.5 md:space-y-2 pr-2">
                {seasons.map((s) => (
                  <div key={s.season}>
                    {seasons.length > 1 && (
                      <div id={`season-${s.season}`} className="text-xs text-muted-foreground font-semibold uppercase tracking-wider px-1 py-2">
                        Saison {s.season}
                      </div>
                    )}
      ...
      {/* Redirect Warning Popup */}
      {
        pendingRedirect && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in">
            <div className="bg-card border border-border/30 p-6 rounded-2xl max-w-md w-full shadow-2xl space-y-4 m-4">
              <div className="flex items-center gap-3 text-yellow-500">
                <AlertCircle className="w-6 h-6" />
                <h3 className="font-bold text-lg">Avertissement de lien externe</h3>
              </div>
              <p className="text-muted-foreground">
                Ce contenu tente d'ouvrir un lien externe.
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                Les sites externes peuvent contenir des publicités ou des traceurs. Soyez prudent.
              </p>
              <div className="flex justify-end gap-3 pt-2">
                <Button variant="ghost" onClick={() => setPendingRedirect(null)}>Annuler</Button>
