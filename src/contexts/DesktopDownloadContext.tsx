...
export function DesktopDownloadProvider({ children }: { children: React.ReactNode }) {
        ...
        if (electron.onDownloadCompleted) {
            electron.onDownloadCompleted(({ episodeId }: { episodeId: string }) => {
                setDownloadStates(prev => {
                    ...
                    if (!current) return prev;
                    toast.success(`Téléchargement terminé : ${current.animeName} - Épisode ${current.episodeNumber}`);
                    return {
                        ...prev,
        ...
        if (electron.onDownloadError) {
            electron.onDownloadError(({ episodeId, error }: { episodeId: string, error: string }) => {
                setDownloadStates(prev => {
                    const current = prev[episodeId];
                    if (!current) return prev;
                    toast.error(`Échec du téléchargement : ${current.animeName}`);
                    return {
                        ...prev,
    ...
    const startDownload = useCallback(async (params: {
        ...
        const electron = (window as any).electron;
        if (!electron) {
            toast.error('Téléchargements disponibles uniquement dans l\'application de bureau');
            return;
        }
        ...
        try {
            ...
            toast.info(`Téléchargement démarré : ${animeName} - Épisode ${episodeNumber}`);
        } catch (error: any) {
            setDownloadStates(prev => ({
                ...prev,
