...
export class MobileUpdateService {
  ...
  private static readonly GITHUB_REPO = 'AniStream';
  ...
  private static mapDbReleaseToGitHub(release: AppRelease): GitHubRelease {
    ...
    const name = release.url.split('/').pop() || `AniStream-${release.version}`;
      ...
      notifications: [
        {
          ...
          body: `AniStream ${release.tag_name} is ready to install`,
          id: 1,
          extra: {
  ...
  private static async downloadAndInstallAPK(release: GitHubRelease): Promise<void> {
    try {
      ...
      // Save to filesystem
      const fileName = `AniStream-${release.tag_name}.apk`;
        ...
        notifications: [
          {
            ...
            body: 'Impossible de télécharger la mise à jour. Veuillez réessayer.',
            id: 4
          }
        ]
      });
  ...
  private static async redirectToWebVersion(): Promise<void> {
    ...
    await LocalNotifications.schedule({
      notifications: [
        {
            title: 'Utiliser la version web',
            body: 'Appuyez sur "Partager" → "Ajouter à l\'écran d\'accueil" pour installer la dernière version',
          id: 5
        }
      ]
    });
