...
const QUEUE_KEY = 'AniStream_mobile_downloads';
const BASE_DIR = 'AniStreamOffline';
...
class MobileDownloadService {
  ...
  private async process() {
    ...
    try {
      ...
      next.eta = undefined;
      logDownload('Téléchargement terminé avec succès', { id: next.id });
      localStorage.removeItem(`dl_url_${next.id}`);

    } catch (err: unknown) {
      const error = err as Error;
      logDownload('Échec du téléchargement', { id: next.id, error: error.message, name: error.name });
      ...
      if (error.name === 'AbortError') {
        ...
        next.status = 'failed';
        next.error = error.message || 'Échec du téléchargement';
        next.speed = undefined;
        next.eta = undefined;
      }
    } finally {
