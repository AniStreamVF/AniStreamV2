import { AniStream_API_URL, getProxiedImageUrl, externalApiGet, withClientHeaders } from "@/lib/api/api-client";
...
export function markProviderSourcesExpired(reason = "expired-stream", ttlMs = PROVIDER_FORCE_REFRESH_TTL_MS): void {
  ...
  if (import.meta.env.DEV) {
    console.warn(`[AniStream] Provider sources marked expired. Forcing fresh fetch for ${ttl}ms (${reason}).`);
  }
}
  ...
  return streams
    ...
    .map((stream: any, index: number) => {
      ...
      if (hasDirect) {
        ...
        url = `${AniStream_API_URL}/techinmind/proxy?url=${encodeURIComponent(stream.dhls)}${
...
function buildMapperBaseCandidates(): string[] {
  ...
  if (configured && !/anime-mapper\.vercel\.app/i.test(configured)) {
    if (configured.startsWith("/api/AniStream/anime/mapper")) {
      add(configured.replace(/^\/api\/AniStream\/anime\/mapper/i, "/api/v2/anime/mapper"));
    } else {
      add(configured);
    }
...
async function providerFetch<T = any>(path: string, timeout = TIMEOUT): Promise<T | null> {
  ...
  try {
    const res = await fetch(`${AniStream_API_URL}${requestPath}`, {
      headers: withClientHeaders({ Accept: "application/json" }),
...
export async function fetchAnimeKaiSources(
  ...
  const toAnimeKaiSources = (json: any, isDub: boolean): StreamingSource[] => {
    ...
    const providerName = `AnimeKai ${isDub ? "VF" : "VOSTFR"}`;
    ...
    servers.forEach((server: any, serverIndex: number) => {
      ...
      entries.forEach((stream: any, sourceIndex: number) => {
        ...
        sources.push({
          ...
          language: stream?.language || server?.language || (isDub ? "VF" : "VOSTFR"),
...
// G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
// Custom Supabase sources (unchanged G�� these go to Supabase, not AniStreamCore)
// G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
            ...
            try {
              ...
              if (result) {
                ...
                if (sources.length > 0) {
                  ...
                  if (import.meta.env.DEV) {
                    console.log(`[AniStream] Background retry succeeded for ${provider.key}: ${sources.length} sources`);
                  }
                  return;
                }
              }
            } catch (err) {
              console.debug(`[AniStream] Background retry failed for ${provider.key}:`, err);
            }
          }
        );
      } finally {
...
export type AniStreamProviderSourceQuery = {
  animeId?: string;
  animeName?: string;
  episodeNumber?: number;
  season?: number;
...
export async function fetchAniStreamProviderSources(
  query: AniStreamProviderSourceQuery
  ...
  if (import.meta.env.DEV) {
    console.log(`[AniStream] Identifier Check:`, {
  ...
  const activeProviderTasks = providerTasks.filter((t) => {
    if (!t.enabled) {
      if (import.meta.env.DEV) {
        ...
        console.log(`[AniStream] Task Disabled: ${t.key} (Reason: ${reason})`);
      }
      return false;
    }
    return true;
  ...
  if (import.meta.env.DEV) {
    console.log(`[AniStream] Enabled providers: ${activeProviderTasks.map(t => t.key).join(", ")}`);
  }
  ...
  const filteredActiveProviderTasks = activeProviderTasks.filter((task) => {
    ...
    if (coolingDown) {
      ...
      if (task.key === "animelok" || task.key === "hindiapi") {
        console.log(`[AniStream] Bypassing cooldown for ${task.key}`);
        return true;
      }
      if (import.meta.env.DEV) {
        console.log(`[AniStream] Task Cooldown Skip: ${task.key}`);
      }
      return false;
    }
    return true;
  ...
  if (filteredActiveProviderTasks.length === 0) {
    ...
    if (cached) {
      if (import.meta.env.DEV) {
        console.log(`[AniStream] Provider aggregation: using cached last-good sources (all providers cooling down)`);
      }
      return cached;
    }
  }
  ...
  for (let attempt = 1; attempt <= PROVIDER_TIMEOUT_RETRY_ATTEMPTS; attempt += 1) {
    ...
    if (import.meta.env.DEV) {
      ...
      console.log(`[AniStream] Timeout retry #${attempt}: ${labels}`);
    }
  ...
  if (import.meta.env.DEV) {
    for (const item of providerDiagnostics) {
      ...
      if (item.status === "error") {
        console.log(`[AniStream] Provider ${label}: error | sources=0 | ${item.error || "unknown"}`);
      } else if (item.status === "timeout") {
        console.log(`[AniStream] Provider ${label}: timeout | sources=0 | ${item.timeoutMs || PROVIDER_EXECUTION_TIMEOUT_MS}ms`);
      } else {
        console.log(`[AniStream] Provider ${label}: ${item.status} | sources=${item.sourceCount}`);
      }
    }
    console.log(
      `[AniStream] Provider aggregation: ${providerCount} provider(s) returned ${sourceCount} total source(s) | ${successfulProviders || "none"}`
    );
  }
  ...
  if (cachedFallback) {
    ...
    if (import.meta.env.DEV) {
      console.log(`[AniStream] Provider aggregation: returning cached last-good sources after empty live result`);
    }
