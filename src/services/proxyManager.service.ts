...
function isMokoProxyUrl(url: string): boolean {
  try {
    ...
    return /(^|\.)moko\.AniStream\.me/i.test(url);
  }
}

const DEFAULT_STREAM_PROXY_PATH = '/api/v1/streamingProxy';
const DEFAULT_NODE_PROXY_LOADER = 'https://hoko.AniStream.me/api/v1/streamingProxy';
...
class ProxyManager {
  ...
  public getOptimalProxy(excludedProxyIds: Set<string> = new Set()): ProxyNode | null {
    ...
    const preferredHoko = available.find((proxy) => proxy.url.toLowerCase().includes('hoko.AniStream.me'));
    if (preferredHoko) {
