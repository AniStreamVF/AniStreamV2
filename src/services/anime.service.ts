import { apiGet, externalApiGet, AniStream_API_URL, API_URL, getProxiedImageUrl, unwrapApiData } from "@/lib/api/api-client";
import type {
  AnimeInfo,
  AnimeCard,
  EpisodeData,
...
const JUSTANIME_PROXY_BASE = String(import.meta.env.VITE_JUSTANIME_PROXY_BASE || `${AniStream_API_URL}/justanime`).replace(/\/+$/, "");
...
function extractUpstreamHost(rawUrl?: string): string {
  ...
  for (let depth = 0; depth < 3; depth += 1) {
    try {
      const parsed = new URL(candidate, "https://AniStream.local");
...
export async function fetchAnimeInfo(
  ...
  if (/^\d+$/.test(animeId)) {
    try {
      const res = await externalApiGet<any>(AniStream_API_URL, `/animelok/anime/${animeId}`);
...
export async function fetchEpisodes(
  ...
  if (/^\d+$/.test(animeId)) {
    ...
    while (hasMore) {
      try {
        const res = await externalApiGet<any>(AniStream_API_URL, `/animelok/watch/${animeId}?ep=${page}`);
        const payload = unwrapApiData<any>(res as any);
...
export async function fetchEpisodeServers(
  ...
  try {
    ...
    let sub: any[], dub: any[], raw: any[];
    // Bridge may return { sub: [...], dub: [...], raw: [...] } directly
    if (serverPayload && typeof serverPayload === 'object' && !Array.isArray(serverPayload) && 'sub' in serverPayload) {
      sub = Array.isArray(serverPayload.sub) ? serverPayload.sub : [];
      dub = Array.isArray(serverPayload.dub) ? serverPayload.dub : [];
      raw = Array.isArray(serverPayload.raw) ? serverPayload.raw : [];
    } else {
      const serversArray: any[] = Array.isArray(serverPayload)
        ...
        : [];
      ({ sub, dub, raw } = Array.isArray(serversArray)
    ...
    if (sub.length === 0 && dub.length === 0 && raw.length === 0) {
      ...
      if (streamRes.ok) {
        ...
        // Check for streamingLink-based server extraction
        if (Array.isArray(streamPayload?.streamingLink) && streamPayload.streamingLink.length > 0) {
          const serverNames = new Set<string>();
          streamPayload.streamingLink.forEach((sl: any) => {
            if (sl.server && !serverNames.has(sl.server)) {
              serverNames.add(sl.server);
              const srv = { serverId: sl.serverId || 1, serverName: sl.server, type: sl.langCode === 'dub' ? 'dub' : 'sub' };
              if (srv.type === 'dub') dub.push(srv);
              else sub.push(srv);
            }
          });
        } else {
    ...
    // Only inject Koro/justanime server when the bridge returned NO real servers
    const hasRealServers = sub.length > 0 || dub.length > 0;
    if (shouldShowJustAnime && !hasRealServers) {
...
export async function fetchStreamingSources(
  ...
  try {
    ...
    if (results) {
      ...
      if (parsed) {
        // Resolve relative proxy URLs (e.g. /proxy/page?url=...) against the bridge origin
        const BRIDGE_ORIGIN = String(import.meta.env.VITE_BACKEND_ORIGIN || 'http://localhost:4567');
        const resolvedSources = (parsed.sources || []).map((s: any) => ({
          ...s,
          url: s.url?.startsWith('/') ? `${BRIDGE_ORIGIN}${s.url}` : s.url,
        }));
        return {
          ...parsed,
          sources: resolvedSources,
          headers: { Referer: results.referer || 'https://megacloud.blog/', 'User-Agent': 'Mozilla/5.0' },
        };
      }
    }
...
export async function fetchAniStreamEpisodeSources(
  episodeId: string,
  server: string = "hd-2",
  category: string = "sub"
