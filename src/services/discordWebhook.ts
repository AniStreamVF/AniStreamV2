import { AniStream_API_URL } from '@/lib/api/api-client';
...
const RAW_AniStream_API_URL = AniStream_API_URL;
...
const AniStream_WEBHOOK_BASE = resolveWebhookBase(RAW_AniStream_API_URL);
...
async function sendToDiscord(payload: WebhookPayload): Promise<void> {
  try {
    // Route through AniStreamAPI which holds the actual webhook URLs
    const res = await fetch(`${AniStream_WEBHOOK_BASE}/webhooks/discord`, {
      method: 'POST',
...
export function notifyUserCreated(user: {
  ...
  const embed: DiscordEmbed = {
    ...
    footer: { text: 'AniStream Auth' },
  };

  void sendToDiscord({
    channel: 'user_created',
    embeds: [embed],
    username: 'AniStream Auth',
  });
}
...
export function notifyError(error: {
  ...
  const embed: DiscordEmbed = {
    ...
    timestamp: new Date().toISOString(),
    footer: { text: 'AniStream Error Logger' },
  };

  void sendToDiscord({
    channel: 'error_logs',
    embeds: [embed],
    username: 'AniStream Errors',
  });
}
...
export function notifyComment(comment: {
  ...
  const embed: DiscordEmbed = {
    ...
    timestamp: new Date().toISOString(),
    footer: { text: 'AniStream Comments' },
  };

  void sendToDiscord({
    channel: 'comment',
    embeds: [embed],
    username: 'AniStream Comments',
  });
}
...
export function notifyReviewPopup(review: {
  ...
  const embed: DiscordEmbed = {
    ...
    timestamp: new Date().toISOString(),
    footer: { text: 'AniStream Review Popup' },
  };

  void sendToDiscord({
    channel: 'review_popup',
    embeds: [embed],
    username: 'AniStream Reviews',
  });
}
