...
export async function resolveMalIdFromEpisodes(animeId: string): Promise<number | null> {
    ...
    try {
        const { fetchEpisodes, fetchAniStreamEpisodeSources, fetchCombinedSources } = await import('@/lib/api');
        ...
        const sourceData = await fetchAniStreamEpisodeSources(firstEpisode.episodeId);
...
/**
 * Maps MAL status strings to AniStream status strings.
 */
export function mapMalStatusToAniStream(malStatus: string): any {
 ...
 */
export function mapMalMangaStatusToAniStream(malStatus: string): any {
 ...
 */
export function mapAniStreamMangaStatusToMal(status: string): any {
...
export async function updateMalMangaStatus(
    ...
    const response = await fetch(functionUrl, {
        ...
        body: JSON.stringify({
            action: 'sync_manga',
            malId,
            status: mapAniStreamMangaStatusToMal(status),
            score,
            numReadChapters,
        })
    });
