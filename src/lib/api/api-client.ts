...
const DEFAULT_API_BASE = "https://api.AniStream.me/api/v2";
...
const CLEAN_API_BASE = CONFIGURED_API_BASE.replace(/\/+(hianime|anime|AniStream)$/i, '');
...
export const AniStream_API_URL = (import.meta.env.DEV && !isMobileNative)
  ? '/api/v2/anime'
...
export async function externalApiGet<T>(baseUrl: string, path: string, retries = 2, timeoutMs: number = 25000): Promise<T> {
  const url = `${baseUrl}${path}`;
  let lastError: Error | null = null;
  const isAniStreamApi = baseUrl === AniStream_API_URL;

  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
      const headers = isAniStreamApi ? await withSignedHeaders(path) : { Accept: 'application/json' };
...
const PROXY_CURSOR_STORAGE_KEY = 'AniStream.streamProxy.cursor';
...
function isHokoProxyCandidate(proxyBaseUrl: string): boolean {
  try {
    ...
    return /(^|\.)hoko\.AniStream\.me$/i.test(parsed.hostname);
  } catch {
    return /(^|\.)hoko\.AniStream\.me/i.test(proxyBaseUrl);
  }
}
...
function isAlreadyGenericM3u8ProxyUrl(url: string): boolean {
  try {
    const parsed = new URL(url, typeof window !== 'undefined' ? window.location.origin : 'https://api.AniStream.me');
...
function extractUpstreamFromProxyUrl(maybeProxyUrl: string): string | null {
  ...
  try {
    const parsed = new URL(
      maybeProxyUrl,
      typeof window !== 'undefined' ? window.location.origin : 'https://api.AniStream.me'
    );
