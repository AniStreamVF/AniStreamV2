      ...
      <div className="mb-6 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm md:text-base text-muted-foreground mt-1.5 font-medium">
            Reprenez vos mangas, manhwas et comics là où vous les avez laissés.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        <GlassPanel className="p-8 border border-white/10 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3 text-muted-foreground">
            ...
            <span className="text-sm font-medium uppercase tracking-wider">Synchronisation de la progression...</span>
          </div>
        </GlassPanel>
          ...
          {continueRows.map((item) => {
            const chapterLabel =
              item.last_chapter_number != null
                ...
                : item.last_chapter_title || "Dernier chapitre";
        ...
        <GlassPanel className="p-8 border border-white/10 flex flex-col justify-center items-center text-center">
          ...
          <p className="text-base text-muted-foreground font-medium max-w-sm">
            {user
              ? "Aucune progression de lecture pour le moment. Commencez à lire un chapitre et il apparaîtra ici."
              : "Connectez-vous pour suivre et reprendre vos dernières lectures manga, manhwa et comics."}
          </p>
          {!user && (
            <Link
              to="/auth"
