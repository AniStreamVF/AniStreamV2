    ...
    if (!logs?.length) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground border-2 border-dashed border-white/10 rounded-xl bg-card/20">
                ...
                <h3 className="text-lg font-bold">Aucune violation trouvée</h3>
                <p className="text-sm">Le système d'auto-modération n'a signalé aucun contenu grave récemment.</p>
            </div>
        );
    }
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                    <div className="flex items-center gap-2">
                                        <p className="font-bold text-sm truncate">{log.profiles?.display_name || log.profiles?.username || 'Utilisateur inconnu'}</p>
