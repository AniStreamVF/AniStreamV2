...
export function PendingForumPosts() {
  ...
  const handleApprove = async (postId: string) => {
    try {
      ...
      toast({ title: 'Publication approuvée avec succès' });
    } catch (error) {
      toast({ title: "Échec de l'approbation de la publication", variant: 'destructive' });
    }
  };
  ...
  const handleReject = async (postId: string) => {
    try {
      ...
      toast({ title: 'Publication rejetée et supprimée' });
    } catch (error) {
      toast({ title: 'Échec du rejet de la publication', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Publications forum en attente</h2>
          <p className="text-sm text-gray-400 mt-1">
            Examiner et approuver les publications avec images
          </p>
        </div>
        <Badge variant="secondary" className="text-lg px-4 py-2">
          {filteredPosts?.length || 0} En attente
        </Badge>
      </div>
      ...
      <div className="relative">
        ...
        <Input
          placeholder="Rechercher par titre, contenu ou utilisateur..."
          value={searchTerm}
      ...
      {isLoading ? (
        <GlassPanel className="p-8 text-center">
          <p className="text-gray-400">Chargement des publications en attente...</p>
        </GlassPanel>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-white">
                        {post.profiles?.display_name || post.profiles?.username || 'Utilisateur inconnu'}
                      </p>
        ...
        <GlassPanel className="p-8 text-center">
          ...
          <p className="text-gray-400">
            {searchTerm ? 'Aucune publication en attente ne correspond à votre recherche' : 'Tout est à jour ! Aucune publication en attente d\'approbation.'}
          </p>
        </GlassPanel>
      )}
    </div>
