...
export function VideoServerManager() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Source vidéo ajoutée avec succès');
      setIsAddDialogOpen(false);
      resetForm();
    },
    onError: (error) => {
      toast.error('Échec de l\'ajout de la source vidéo : ' + error.message);
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['custom-video-sources'] });
      toast.success('Source vidéo mise à jour');
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['custom-video-sources'] });
      toast.success('Source vidéo supprimée');
    },
    onError: (error) => {
  ...
  // Search anime for autocomplete - prioritize internal API for correct AniStream IDs
  const [searchResults, setSearchResults] = useState<Array<{ id: string; name: string; image?: string; source: 'internal' | 'jikan' }>>([]);
  ...
  const searchAnime = async (query: string) => {
    ...
    try {
      // Try internal HiAnime API first (for correct AniStream IDs)
      ...
      if (internalRes.ok) {
        ...
        if (animes.length > 0) {
          ...
          toast.success(`${results.length} anime trouvés (IDs AniStream)`);
          return;
        }
      }

      // Fallback to Jikan API (for reference only - IDs won't link correctly in AniStream)
      ...
      if (jikanRes.ok) {
        ...
        if (jikanData.data?.length > 0) {
          ...
          setSearchResults(results);
          toast.info('Résultats de MAL — l\'anime peut ne pas être correctement lié dans le lecteur');
          return;
        }
      }

      toast.error('Aucun anime trouvé');
    } catch (error) {
      console.error('Search error:', error);
      toast.error('Recherche échouée — essayez de saisir l\'ID de l\'anime manuellement');
    } finally {
  ...
  const selectAnime = (anime: { id: string; name: string }) => {
    ...
    setSearchResults([]);
    toast.success(`Sélectionné : ${anime.name}`);
  };
  ...
  const handleDelete = (id: string) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer cette source vidéo ?')) {
...
Ajouter une source vidéo
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Ajouter une source vidéo personnalisée</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <div className="pt-2 border-t border-border/50">
                  <Label className="text-xs text-muted-foreground">Ou saisissez l'ID de l'anime manuellement :</Label>
                  <Input
                    placeholder="Entrez l'ID de l'anime..."
              ...
              <div className="space-y-2">
                ...
                <Input
                  placeholder="ex. HD personnalisé, Serveur de secours"
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder="URL vidéo directe ou URL embed"
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder="URL VTT ou SRT"
                  value={newSource.subtitle_url}
                  onChange={(e) => setNewSource({ ...newSource, subtitle_url: e.target.value })}
                  rows={2}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={newSource.subtitle_lang}
                  onChange={(e) => setNewSource({ ...newSource, subtitle_lang: e.target.value })}
                />
              </div>
              ...
              {/* Priority */}
              <div className="space-y-2">
                <Label>Priorité (plus bas = plus prioritaire)</Label>
                <Input
                  type="number"
                  min={1}
                  max={100}
      ...
      <div className="relative">
        ...
        <Input
          placeholder="Rechercher par anime ou serveur..."
          value={searchQuery}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">Aucune source vidéo personnalisée</p>
          <p className="text-sm text-muted-foreground mt-1">
            Cliquez sur "Ajouter une source vidéo" pour ajouter un serveur vidéo personnalisé pour n'importe quel anime
          </p>
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            <span>•</span>
                            <span className={source.subtitle_url ? 'text-primary' : ''}>
                              {source.subtitle_url ? `VOSTFR : ${source.subtitle_lang || 'Oui'}` : 'Pas de VOSTFR'}
                            </span>
                            <span>•</span>
