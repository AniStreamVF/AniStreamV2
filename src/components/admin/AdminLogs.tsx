...
export function AdminLogs() {
  ...
  const handleCopyErrorId = async (errorId?: string) => {
    if (!errorId) {
      toast({ title: 'Aucun ID d\'erreur', description: 'Ce log ne contient pas d\'ID d\'erreur.' });
      return;
    }
    try {
      await navigator.clipboard.writeText(errorId);
      toast({ title: 'Copié', description: `ID d'erreur copié ${errorId}` });
    } catch (e) {
      toast({ title: 'Échec de la copie', description: String(e) });
    }
  };

  const handleDeleteFiltered = async () => {
    if (!filteredLogs || filteredLogs.length === 0) {
      toast({ title: 'Rien à supprimer', description: 'Aucun log ne correspond aux filtres actuels.' });
      return;
    }

    const confirmMsg = `Supprimer ${filteredLogs.length} logs ? Cette action est irréversible.`;
    if (!confirm(confirmMsg)) return;

    const ids = filteredLogs.map((l) => l.id);
    try {
      await deleteMutation.mutateAsync({ ids });
      toast({ title: 'Supprimé', description: `Supprimé ${ids.length} logs.` });
    } catch (e: any) {
      toast({ title: 'Échec de la suppression', description: e?.message || String(e) });
    }
  };

  const handleCleanupOld = async () => {
    const cutoff = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString();
    const confirmMsg = 'Supprimer les logs de plus de 2 jours ? Cette action est irréversible.';
    if (!confirm(confirmMsg)) return;

    try {
      await deleteMutation.mutateAsync({ created_before: cutoff });
      toast({ title: 'Nettoyage terminé', description: 'Anciens logs supprimés.' });
    } catch (e: any) {
      toast({ title: 'Échec du nettoyage', description: e?.message || String(e) });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Journaux d'activité admin</h2>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-white">
                          {log.profiles?.display_name || log.profiles?.username || 'Utilisateur inconnu'}
                        </p>
                  ...
                  <div className="mt-2 space-y-1">
                    <p className="text-sm text-gray-300">
                      <span className="text-gray-500">Type d'entité :</span>{' '}
                        ...
                        {log.details.error_id && (
                          <div className="flex items-center gap-2">
                            <p className="text-xs text-gray-400 font-mono">
                              <span className="text-gray-500">ID d'erreur :</span> {log.details.error_id}
                            </p>
                            <button
