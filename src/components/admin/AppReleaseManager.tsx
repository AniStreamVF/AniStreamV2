...
export function AppReleaseManager() {
    ...
    const saveMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success(editingId ? 'Version mise à jour' : 'Version ajoutée');
            resetForm();
        },
        onError: (error) => {
            console.error('Save release error:', error);
            toast.error('Échec de la sauvegarde de la version');
        }
    });
    ...
    const deleteMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error('Échec de la suppression de la version');
        }
    });
        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <Button
                                    ...
                                    size="sm"
                                    onClick={() => {
                                        if (confirm('Supprimer cette version ?')) deleteMutation.mutate(release.id);
                                    }}
                ...
                <div className="text-center py-12 rounded-xl border border-dashed border-white/10 text-muted-foreground">
                    ...
                    <p>Aucune version trouvée dans la base de données.</p>
                </div>
            )}
        </div>
    );
