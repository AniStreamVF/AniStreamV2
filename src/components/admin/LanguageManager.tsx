...
export function LanguageManager() {
    ...
    const addAnimeToLanguage = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['admin_custom_languages'] });
            toast.success("Anime ajouté à la section !");
        ...
        <div className="space-y-8 pb-20">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Gérer le système de langue décentralisé</p>
                </div>
                {!isAddingLanguage && (
                ...
                <GlassPanel className="p-6 border-primary/20 bg-primary/5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className="space-y-2">
                            ...
                            <Input placeholder="ex. Hindi Dubbed" value={newLangName} onChange={(e) => setNewLangName(e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs font-black uppercase text-muted-foreground">Code slug</label>
                            <Input placeholder="ex. hindi" value={newLangCode} onChange={(e) => setNewLangCode(e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs font-black uppercase text-muted-foreground">URL de bannière (optionnelle)</label>
                            <Input placeholder="https://..." value={newLangPoster} onChange={(e) => setNewLangPoster(e.target.value)} />
                        </div>
                    </div>
                    <div className="flex justify-end gap-3">
                        <Button variant="ghost" onClick={() => setIsAddingLanguage(false)}>Annuler</Button>
                        ...
                        <GlassPanel key={lang.id} className="overflow-hidden border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all">
                            <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button variant="ghost" size="sm" onClick={() => {
                                        if (confirm(`Supprimer ${lang.name} et tous ses animes ?`)) deleteLanguage.mutate(lang.id);
                            ...
                            <div className="p-6">
                                {lang.anime && lang.anime.length > 0 ? (
                                    ...
                                    <div className="py-10 text-center border-2 border-dashed border-white/5 rounded-3xl">
                                        <p className="text-muted-foreground text-sm font-medium">Aucun anime ajouté à cette section.</p>
                                    </div>
                                )}
                            </div>
                        </GlassPanel>
                        ...
                        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                            <div className="flex gap-4">
                                <div className="flex-1 space-y-2">
                                    <label className="text-xs font-black uppercase text-muted-foreground">Rechercher un anime</label>
                                    <div className="relative">
                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                                        <Input
                                            placeholder="Rechercher sur Animelok..."
                                ...
                                <div className="w-40 space-y-2">
                                    <label className="text-xs font-black uppercase text-muted-foreground">Horaire de diffusion</label>
                                    <Input
