...
function toPreviewCard(item: CuratedManualItem): UnifiedMediaCardProps["item"] {
  ...
  return {
    ...
    name: String(item.title || "Sans titre").trim() || "Sans titre",
    poster,
...
export function CuratedHomeManager() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(editingId ? "Section mise à jour" : "Section créée");
      setEditingId(null);
      setDraft(emptyDraft());
      setManualItemDraft(emptyManualItem("anime"));
    },
    onError: (error: any) => {
      toast.error(error?.message || "Échec de l'enregistrement de la section");
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => toast.error("Échec de la suppression de la section"),
  });
  ...
  const toggleActiveMutation = useMutation({
    ...
    onError: () => toast.error("Échec du changement de statut de la section"),
  });
  ...
  const addManualItem = () => {
    ...
    if (!title) {
      toast.error("Le titre manuel de l'élément est requis");
      return;
    }
    if (!routeId) {
      toast.error("Le slug interne anime/manga est requis");
      return;
    }
    if (/^https?:\/\//i.test(routeId)) {
      toast.error("Utilisez le slug interne dans l'ID de route, pas une URL externe");
      return;
    }

    const linkValue = manualItemDraft.link.trim();
    if (linkValue && /^https?:\/\//i.test(linkValue)) {
      toast.error("Utilisez le chemin interne dans le lien (ex: /anime/slug), pas une URL externe");
      return;
    }
  ...
  const applyQuickAdd = () => {
    ...
    if (lines.length === 0) {
      toast.error("Collez au moins une ligne pour un ajout rapide");
      return;
    }
    ...
    if (parsedItems.length === 0) {
      toast.error("Aucune ligne valide trouvée. Utilisez : titre|slug-interne|couverture(optionnel)|type(optionnel)");
      return;
    }
    ...
    setQuickAddInput("");
    toast.success(`${parsedItems.length} élément${parsedItems.length > 1 ? "s" : ""} ajouté(s)`);
  };
  ...
  const handleSubmit = () => {
    ...
    if (metadata.manualItems.length === 0) {
      toast.error("Ajoutez au moins un élément anime/manga à cette section");
      return;
    }
    ...
    <div className="space-y-3">
      ...
      {rows.length === 0 ? (
        <GlassPanel className="p-4 border border-dashed border-white/10">
          <p className="text-sm text-muted-foreground">Aucune section organisée pour {scope}.</p>
        </GlassPanel>
      ) : (
        rows.map((section) => {
            ...
            <GlassPanel key={section.id} className="p-4 border border-white/10">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleEdit(section)} title="Modifier la section">
    ...
    <div className="space-y-8">
      <GlassPanel className="p-6 border-primary/20 bg-primary/5">
        <div className="flex items-center justify-between gap-4 mb-5">
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2">
              ...
              Constructeur de page d'accueil (Manuel)
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            >
              <option value="anime">Page d'accueil Anime</option>
              <option value="manga">Page d'accueil Manga</option>
            </select>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <Input
              ...
              placeholder="Exemple : Sélections de l'éditeur"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <Textarea
              ...
              placeholder="Sous-titre optionnel affiché sous le titre"
            />
          </div>
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          {showAdvancedFields ? (
            ...
            <Input
              ...
              placeholder="URL de couverture (manuel)"
            />
            <Input
            ...
            <Input
              ...
              placeholder="ID AniList (optionnel)"
            />
            <Input
              value={manualItemDraft.malId}
              onChange={(event) => setManualDraftField("malId", event.target.value)}
              placeholder="ID MAL (optionnel)"
            />
          </div>
          ) : null}

          <div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-3">
            <p className="text-xs font-semibold text-foreground">Ajout rapide multiple</p>
            <p className="text-[11px] text-muted-foreground mt-1">
              Un par ligne. Format : <span className="text-foreground">titre|slug-interne|couverture(optionnel)|type(optionnel anime/manga)|anilist(optionnel)|mal(optionnel)|lien(optionnel chemin interne)</span>
            </p>
            <Textarea
          ...
          {metadata.manualItems.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground mt-3">Aucun élément manuel...</p>
          )}
        </div>
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.02] p-4">
          ...
          {previewCards.length === 0 ? (
            <p className="text-xs text-muted-foreground">Ajoutez des éléments manuels pour prévisualiser l'apparence de cette section.</p>
          ) : (
        ...
        <div className="mt-5 flex items-center gap-3">
          <Button onClick={handleSubmit} disabled={saveMutation.isPending || !draft.title?.trim()}>
            ...
            {editingId ? "Mettre à jour la section" : "Créer une section"}
          </Button>
          <Button
            variant="outline"
          ...
          {editingId ? (
            <Button variant="outline" onClick={handleCancelEdit}>Réinitialiser</Button>
          ) : null}
        </div>
      </GlassPanel>
      ...
      {isLoading ? (
        <GlassPanel className="p-6">
          <p className="text-sm text-muted-foreground">Chargement des sections organisées...</p>
        </GlassPanel>
      ) : (
