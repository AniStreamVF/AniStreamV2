...
const REMOTE_NODE_STREAM_PROXY = 'https://hoko.AniStream.me/api/v1/streamingProxy';
...
function isMokoProxyUrl(value: string): boolean {
  try {
    const parsed = new URL(value);
    return /(^|\.)moko\.AniStream\.me$/i.test(parsed.hostname);
  } catch {
    return /(^|\.)moko\.AniStream\.me/i.test(value);
  }
}
...
export function VideoPlayer({
  ...
  const AniStreamSkipTimes = useMemo(() => {
    ...
    if (introWindow && Number.isFinite(introWindow.start) && Number.isFinite(introWindow.end) && introWindow.end > introWindow.start) {
      windows.push({
        ...
        skipType: 'op',
        skipId: `AniStream-op-${episodeId || episodeNumber || 'current'}`,
        episodeLength: 0,
      });
    }
    ...
    if (outroWindow && Number.isFinite(outroWindow.start) && Number.isFinite(outroWindow.end) && outroWindow.end > outroWindow.start) {
      windows.push({
        ...
        skipType: 'ed',
        skipId: `AniStream-ed-${episodeId || episodeNumber || 'current'}`,
        episodeLength: 0,
      });
    }
  ...
  const hasAniStreamSkipWindows = AniStreamSkipTimes.length > 0;
  const effectiveSkipTimes = hasAniStreamSkipWindows ? AniStreamSkipTimes : skipTimes;
  const hasAniStreamSkipWindowsRef = useRef(hasAniStreamSkipWindows);

  useEffect(() => {
    hasAniStreamSkipWindowsRef.current = hasAniStreamSkipWindows;
  }, [hasAniStreamSkipWindows]);
  ...
  const PROGRESS_SAVE_INTERVAL = 2; // seconds (was 15, reduced for watch party sync)
  ...
  const handleCustomSubtitleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success(`Sous-titre chargé : ${file.name}`);
  };
  ...
  useEffect(() => {
    if (isNative && (window as any).electron && animeName) {
      ...
      const extra: any = {
        ...
        smallImageText: isPlaying ? 'En cours' : 'En pause'
      };

      (window as any).electron.updateRPC({
        details: `Regarde ${animeName}`,
        state: `Épisode ${episodeNumber || '...'}`,
        extra
      });
    }
  ...
  useEffect(() => {
    ...
    if (hasAniStreamSkipWindows) return;
  ...
  }, [malId, episodeNumber, fetchSkipTimes, hasAniStreamSkipWindows]);
  ...
  const loadVideo = useCallback(
        ...
        const trySwitchProxyCandidate = async (context?: { statusCode?: number }) => {
          ...
          try {
            for (let candidateIndex = activeProxyIndex + 1; candidateIndex < proxyCandidates.length; candidateIndex += 1) {
              ...
              hls.startLoad();
              toast.success(`Route proxy changée (${candidateIndex + 1}/${proxyCandidates.length})`, { id: 'proxy-failover' });
              return true;
            }
    ...
    audioCodecSwapAttemptedRef.current = false;
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
          ...
          if (!hasAniStreamSkipWindowsRef.current && malId && episodeNumber) {
        ...
        hls.on(Hls.Events.ERROR, async (_, data) => {
          ...
          if (data.response?.code === 0 || data.details === 'manifestLoadError') {
            toast.error("Lecture bloquée ? Essayez de désactiver la 'Prévention de pistage' ou de changer de serveur.", { duration: 5000, id: 'tracking-prevention' });
          }
          ...
          if (data.fatal) {
            ...
            switch (data.type) {
              ...
              default:
                ...
                setVideoError("Échec du chargement de la vidéo");
  ...
  const handleScreenshot = () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        toast.success(`Capture d'écran enregistrée : ${filename}`);
      }, 'image/png');
    } catch (err) {
  ...
  return (
    ...
    <div
      ...
      className="relative aspect-video bg-black text-white rounded-xl md:rounded-2xl overflow-hidden group touch-manipulation video-player-container video-player-modern"
          ...
          <div className="flex flex-col items-center gap-4 text-center p-6 md:p-10 max-w-md">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-black/40 hover:bg-black/60 disabled:opacity-50 disabled:cursor-not-allowed backdrop-blur-sm flex items-center justify-center gap-2 transition-all font-medium video-controls-btn"
        ...
        <div className="flex items-center justify-between gap-3 md:gap-4">
          ...
          <div className="flex items-center gap-1.5 md:gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-xl bg-black/40 hover:bg-black/60 backdrop-blur-sm transition-all video-controls-btn"
            ...
            {!isLive && (
              ...
              <>
                ...
                <button
                  ...
                  className="p-2 rounded-xl bg-black/40 hover:bg-black/60 backdrop-blur-sm transition-all hidden sm:flex video-controls-btn"
            ...
            <div className="hidden md:flex items-center gap-2 group/volume ml-1">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-black/40 hover:bg-black/60 backdrop-blur-sm transition-all video-controls-btn"
                ...
                {showSubtitleMenu && (
                  ...
                  <div className="absolute bottom-full right-0 mb-2 bg-background/95 backdrop-blur-sm border border-border rounded-xl p-2 min-w-[120px] md:min-w-[150px] max-h-[400px] overflow-y-auto pointer-events-auto">
                    ...
                    <label className="w-full px-3 py-1.5 text-left text-xs md:text-sm rounded-lg hover:bg-muted transition-colors text-foreground flex items-center gap-2 cursor-pointer">
                      ...
                      <span>Ajouter personnalisé</span>
                      <input
                        type="file"
                        accept=".vtt,.srt"
                        className="hidden"
