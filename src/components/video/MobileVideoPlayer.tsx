...
export function MobileVideoPlayer({
  ...
  const AniStreamSkipSegments = useMemo<SkipSegment[]>(() => {
  ...
  const hasAniStreamSkipSegments = AniStreamSkipSegments.length > 0;
  const effectiveSkipSegments = hasAniStreamSkipSegments ? AniStreamSkipSegments : aniskipSegments;
  ...
  useEffect(() => {
    if (hasAniStreamSkipSegments) return;
    if (malId && episodeNumber && duration > 0) {
      fetchSkipTimes(malId, episodeNumber, Math.floor(duration));
    }
  }, [malId, episodeNumber, duration, fetchSkipTimes, hasAniStreamSkipSegments]);
      ...
      hls.on(Hls.Events.ERROR, (_, data) => {
        if (data.fatal) {
          if (retryCountRef.current < 3) {
            ...
            setVideoError("Échec du chargement de la vidéo. Veuillez essayer un autre serveur.");
          }
        }
      });
  ...
  const handleDownload = async () => {
    if (!animeId || !episodeId || !animeName) {
      toast.error("Impossible de télécharger : informations manquantes");
      return;
    }
    const source = sources[0];
    if (!source) {
      toast.error("Aucune source vidéo disponible");
      return;
    }
    await startDownload({
      animeId,
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full aspect-video bg-black text-white select-none touch-none"
      ...
      {showControls && !isLocked && (
        <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-transparent to-black/90">
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/80 text-xs truncate">
                    Épisode {episodeNumber} {episodeTitle ? `• ${episodeTitle}` : ''}
                  </p>
                </div>
              </div>
          ...
          {activeSkip && (
            ...
            >
              {activeSkip.type === 'op' || activeSkip.type === 'mixed-op' ? 'Passer l\'intro' : activeSkip.type === 'recap' ? 'Passer le recap' : 'Passer l\'outro'}
            </button>
          )}
                ...
                <div className="relative">
                  ...
                  {showSettingsMenu && (
                    <div className="absolute bottom-14 right-0 bg-black/95 backdrop-blur-xl rounded-2xl p-2 min-w-[160px] border border-white/10 shadow-2xl">
                      <p className="px-4 py-2 text-white/50 text-xs font-semibold uppercase tracking-wide">Vitesse de lecture</p>
