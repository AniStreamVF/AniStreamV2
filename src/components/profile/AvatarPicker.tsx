...
export function AvatarPicker({ type, trigger, currentImage }: AvatarPickerProps) {
  ...
  const handleSelect = async () => {
    ...
    try {
      ...
      toast.success(`${type === 'avatar' ? 'Avatar' : 'Bannière'} mis à jour !`);
      setOpen(false);
      setSelectedImage(null);
    } catch (error) {
      toast.error(`Échec de la mise à jour ${type === 'avatar' ? "de l'avatar" : "de la bannière"}`);
    }
  };
      ...
      <DialogContent className="sm:max-w-2xl bg-background/95 backdrop-blur-xl border-border/50">
        <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {type === 'avatar' ? 'Choisir un avatar' : 'Choisir une bannière'}
            </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              Sélectionnez une image de style anime
            </p>
            <Button
              variant="outline"
              size="sm"
          ...
          {images?.length === 0 && !isLoading && (
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p>Aucune image trouvée. Essayez d'actualiser.</p>
            </div>
          )}
