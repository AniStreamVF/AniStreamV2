...
export function LogViewer() {
  ...
  const exportLogs = () => {
    ...
    a.href = url;
    a.download = `AniStream-logs-${new Date().toISOString().split('T')[0]}.txt`;
    a.click();
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-gradient-to-r from-zinc-900/80 to-zinc-900/40">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="icon" onClick={() => setLogs([])} title="Effacer" className="h-8 w-8 hover:bg-white/5">
                <Trash2 className="w-4 h-4 text-zinc-400" />
              </Button>
              <Button variant="ghost" size="icon" onClick={copyLogs} title="Copier" className="h-8 w-8 hover:bg-white/5">
                <Copy className="w-4 h-4 text-zinc-400" />
              </Button>
              <Button variant="ghost" size="icon" onClick={exportLogs} title="Exporter" className="h-8 w-8 hover:bg-white/5">
                <Download className="w-4 h-4 text-zinc-400" />
              </Button>
              <div className="w-px h-5 bg-white/10 mx-1" />
              <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} title="Fermer (Échap)" className="h-8 w-8 hover:bg-red-500/20 hover:text-red-400">
                <X className="w-4 h-4" />
              </Button>
            </div>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 font-mono text-xs">
            ...
            {filteredLogs.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-zinc-700 space-y-3 py-20">
                ...
                <p className="text-sm">Aucun log capturé pour le moment</p>
                <p className="text-xs text-zinc-600">La sortie console apparaîtra ici</p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between px-4 py-2 border-t border-white/5 bg-zinc-900/30 text-[10px] text-zinc-600">
            <span>Appuyez sur <kbd className="px-1.5 py-0.5 bg-white/5 rounded text-zinc-400">F12</kbd> ou <kbd className="px-1.5 py-0.5 bg-white/5 rounded text-zinc-400">Ctrl+Shift+I</kbd> pour basculer</span>
