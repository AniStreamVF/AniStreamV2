...
export function TierListCard({ tierList, showAuthor = true, onShare }: TierListCardProps) {
  ...
  const handleLike = async () => {
    if (!user) {
      toast.error('Connectez-vous pour aimer les classements');
      return;
    }
    ...
    try {
      ...
      toast.error('Échec du like du classement');
    }
  };
...
export function TierListGrid({ tierLists, showAuthor = true, emptyMessage = 'Aucun classement pour le moment' }: TierListGridProps) {
