...
const TIERLIST_DRAFT_STORAGE_PREFIX = 'AniStream_tierlist_editor_draft_v1';
...
export function TierListEditor({ initialData, onSave, onClose }: TierListEditorProps) {
  ...
  // Search anime or characters using AniStream API
  ...
  const handleLoadMore = () => {
    ...
    setPage(nextPage);
    performSearch(searchQuery, searchType, nextPage);
  };
  ...
  const addItemToTier = (item: SearchResult, tier: string) => {
    ...
    if (items.some(i => i.anime_id === item.id)) {
      toast.error('Cet élément est déjà dans votre classement');
      return;
    }
    ...
    toast.success(`${item.title} ajouté au rang ${tier}`);
  };
  ...
  const handleSave = useCallback(async () => {
    if (!name.trim()) {
      toast.error('Veuillez entrer un nom pour votre classement');
      return;
    }
    ...
    try {
      if (initialData?.id) {
        ...
        toast.success('Classement créé !');
      }
      clearSavedDraft();
      onSave?.({ name, description, items, is_public: isPublic });
      onClose?.();
    } catch (error: any) {
      const message = error?.message || error?.details || 'Échec de l\'enregistrement du classement';
      toast.error(message);
    }
  }, [
    clearSavedDraft,
    ...
    <div className="space-y-6">
      ...
      <GlassPanel className="p-4">
        ...
        <div className="relative mb-4">
          ...
          <Input
            placeholder={searchType === 'anime' ? "Rechercher un anime (min 3 car.)..." : "Rechercher un personnage (min 3 car.)..."}
            value={searchQuery}
      ...
      <div className="flex justify-end gap-4">
        ...
        >
          ...
          {initialData?.id ? 'Mettre à jour' : 'Enregistrer'} le classement
        </Button>
      </div>
    </div>
  );
