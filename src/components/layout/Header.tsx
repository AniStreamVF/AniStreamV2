...
export function Header() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (hasModifier && e.key.toLowerCase() === 'k') {
        ...
        const ids = ['AniStream-header-search', 'AniStream-global-search', 'AniStream-search-mobile-input'];
        for (const id of ids) {
  ...
  const getStatusTooltip = () => {
    switch (systemStatus) {
      ...
      case 'degraded':
        return 'Certains services dégradés';
      default:
      ...
      <div className="flex items-center justify-between w-full gap-8">
        <div className="flex items-center gap-3">
          <h2 className="text-muted-foreground text-sm font-medium tracking-wider uppercase">
            Bon retour, <span className="text-foreground">{profile?.display_name || 'Voyageur'}</span>
          </h2>
          {isBanned && (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">Statut</span>
          </Link>
        </div>
        ...
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Rechercher un anime par image
            </DialogTitle>
            <DialogDescription>
              Importez une capture d'écran ou une image d'un anime pour l'identifier via trace.moe
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
              ...
              <label className="flex-1 cursor-pointer group">
                <div className="flex items-center justify-center gap-2 px-4 h-12 rounded-xl bg-muted/50 border-2 border-dashed border-white/10 group-hover:border-primary/50 transition-colors">
                  ...
                  <span className="text-sm text-muted-foreground group-hover:text-foreground transition-colors overflow-hidden truncate">
                    {selectedFile ? selectedFile.name : 'Choisir une capture d\'écran...'}
                  </span>
                  <input
                    ref={fileInputRef}
                    type="file"
            ...
            {imageResults && imageResults.length > 0 && (
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                <h3 className="text-sm font-bold flex items-center gap-2">
                  ...
                  {imageResults.length} résultat trouvé
                </h3>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${parseFloat(similarity) > 90 ? 'bg-green-500/20 text-green-500' : parseFloat(similarity) > 80 ? 'bg-yellow-500/20 text-yellow-500' : 'bg-red-500/20 text-red-500'}`}>
                          {similarity}% de correspondance
                        </span>
                      </div>
                      {result.episode && (
                        <p className="text-xs text-muted-foreground mb-3">
                          Épisode {result.episode} • {Math.floor(result.from / 60)}:{Math.floor(result.from % 60).toString().padStart(2, '0')}
                        </p>
                      )}
                      <div className="flex gap-2">
                        <button
            ...
            {imageResults && imageResults.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Aucune correspondance trouvée. Essayez une autre image.</p>
              </div>
            )}
          </div>
        </DialogContent>
