...
export function TitleBar() {
  ...
  const [title, setTitle] = useState("AniStream");
  ...
  useEffect(() => {
    const updateTitle = () => {
        if (location.pathname === '/') setTitle('AniStream');
        else if (location.pathname === '/setup') setTitle('Setup');
        else if (location.pathname === '/offline') setTitle('Téléchargements');
        else if (location.pathname === '/settings') setTitle('Paramètres');
        else setTitle(document.title?.replace(' | AniStream', '') || 'AniStream');
    };
    
    updateTitle();
    
    const observer = new MutationObserver(() => {
        setTitle(document.title?.replace(' | AniStream', '') || 'AniStream');
    });
