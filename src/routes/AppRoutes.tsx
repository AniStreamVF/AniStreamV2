...
const TrailerPage = lazy(() => import("../pages/base/TrailerPage"));
...
export function GlobalListeners() {
  ...
  const isSetupComplete = localStorage.getItem('AniStream_setup_complete') === 'true';
  ...
  useEffect(() => {
    if (isLoading) return;
    const onboardingComplete = localStorage.getItem('AniStream_onboarding_complete') === 'true';
    const themeSelected = localStorage.getItem('AniStream_theme_selected_v2') === 'true';
  ...
  return (
    <Suspense fallback={<PageLoader />}>
      ...
      <Routes>
        ...
        <Route path="/watch/:episodeId" element={<ProtectedRoute><KeyedWatchPage /></ProtectedRoute>} />
        ...
        <Route path="/v2" element={<TrailerPage />} />
...
// Wrapper to force remount WatchPage when episodeId changes
function KeyedWatchPage() {
  const { episodeId } = useParams<{ episodeId: string }>();
  const location = useLocation();
  return <WatchPage key={`${episodeId}-${location.search}`} />;
}
