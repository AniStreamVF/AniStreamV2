...
export async function fetchExternalIds(
  ...
  try {
    ...
    const { fetchEpisodes, fetchAniStreamEpisodeSources } = await import('@/lib/api');
    
    const episodesData = await fetchEpisodes(animeId);
    if (episodesData?.episodes?.[0]) {
      const sourceData = await fetchAniStreamEpisodeSources(episodesData.episodes[0].episodeId);
