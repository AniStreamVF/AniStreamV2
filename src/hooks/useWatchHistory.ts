...
import { updateAniListAnimeStatus, mapAniStreamStatusToAniList } from '@/lib/externalIntegrations';
...
export function useWatchHistory(limit?: number) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error('[useWatchHistory] Query error:', error);
        throw error;
      }
      console.debug('[useWatchHistory] Fetched', data?.length ?? 0, 'entries for user', user?.id);
    ...
    onSuccess: (data, variables) => {
      ...
      if (profile?.anilist_access_token) {
        ...
        if (isStart || isEnd) {
          const syncStatus = (isEnd && variables.isLastEpisode) ? 'completed' : 'watching';
          const episodeToSync = variables.episodeNumber;
          const aniListStatus = mapAniStreamStatusToAniList(syncStatus);
