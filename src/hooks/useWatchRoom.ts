...
export function useInfinitePublicWatchRooms() {
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 0 }) => {
            ...
            // Count only participants seen within the last 45s
            ...
            // Only return rooms that have at least one active participant
            const activeRooms = rooms.filter(r => (countMap.get(r.id) || 0) > 0);
...
export function useWatchRoom(roomId: string | undefined) {
    return useQuery({
        ...
        refetchInterval: (query) => {
            ...
            return room.is_playing ? 2000 : false;
        },
    });
}

// Fetch room participants (filters out stale ghosts >45s since last_seen_at)
...
export function useJoinRoom() {
    ...
    return useMutation({
        mutationFn: async ({ roomId, password }: { roomId: string; password?: string }) => {
            ...
            // Check participant count (ignore stale ghosts)
            const staleThreshold = new Date(Date.now() - 45000).toISOString();
            ...
            // Join room (update if exists, insert if not)
...
// Leave a room (auto-closes if host or last participant)
    ...
    return useMutation({
        mutationFn: async (roomId: string) => {
            ...
            if (isHost) {
                ...
                // Guest leaves → remove themselves, then check if room is now empty
                const { error } = await supabase
                    .from('watch_room_participants')
                    .delete()
                    .eq('room_id', roomId)
                ...
                // If no active participants remain, auto-close the room
                const staleThreshold = new Date(Date.now() - 45000).toISOString();
                const { count } = await supabase
                    .from('watch_room_participants')
                    .select('*', { count: 'exact', head: true })
