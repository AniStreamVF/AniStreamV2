...
function getSessionId(): string {
  let sessionId = localStorage.getItem('AniStream_session_id');
  if (!sessionId) {
    sessionId = crypto.randomUUID();
    localStorage.setItem('AniStream_session_id', sessionId);
  }
  return sessionId;
}

const VISITOR_INFO_CACHE_KEY = 'AniStream_visitor_info_cache_v1';
...
export function usePageTracking(enabled: boolean = true) {
  ...
  useEffect(() => {
    ...
    const locationChangeEvent = 'AniStream:location-change';
