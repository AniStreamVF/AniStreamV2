...
import { updateAniListAnimeStatus, mapAniStreamStatusToAniList, disconnectAniList } from '@/lib/externalIntegrations';
...
export function useAddToWatchlist() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      toast.success('Liste de suivi mise à jour');
        ...
        if (targetMalId) {
          ...
          updateMalAnimeStatus(targetMalId, variables.status || 'plan_to_watch')
            .then(() => {
              ...
              toast.success('Synchronisé vers MyAnimeList');
            })
            .catch(err => {
              console.error('[Watchlist Sync] ✗ MAL sync FAILED:', err);
              toast.error('Échec de la synchronisation vers MyAnimeList');
            });
        } else {
      ...
      if (profile?.anilist_access_token) {
        ...
        if (targetAnilistId) {
          ...
          const aniStatus = mapAniStreamStatusToAniList(variables.status || 'plan_to_watch');
          ...
          updateAniListAnimeStatus(profile.anilist_access_token, Number(targetAnilistId), aniStatus)
            .then(() => {
              ...
              toast.success('Synchronisé vers AniList');
            })
            .catch(err => {
              console.error('[Watchlist Sync] ✗ AniList sync FAILED:', err);
              toast.error('Échec de la synchronisation vers AniList');
            });
        } else {
    ...
    onError: (error: any) => {
      ...
      toast.error(`Échec de la mise à jour de la liste de suivi : ${error?.message || 'Erreur inconnue'}`);
    },
  });
}
...
export function useRemoveFromWatchlist() {
  ...
  return useMutation({
    ...
    onSuccess: (_, animeId) => {
      ...
      toast.success('Retiré de la liste de suivi');
    },
    onError: () => {
      toast.error('Échec du retrait de la liste de suivi');
    },
  });
}
...
export function useBulkRemoveFromWatchlist() {
  ...
  return useMutation({
    ...
    onSuccess: (_, animeIds) => {
      ...
      toast.success('Éléments retirés de la liste de suivi');
    },
    onError: (error: any) => {
      toast.error(`Échec du retrait : ${error?.message || String(error)}`);
    },
  });
}
