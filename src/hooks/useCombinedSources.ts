...
/**
 * Combined hook that fetches AniStreamAPI, HiAnime, WatchAnimeWorld, and AnimeHindiDubbed sources
 */
export function useCombinedSources(
  ...
  return useQuery<StreamingData & {
    ...
    hasAniStreamAPI: boolean;
    hasAniStreamProviders: boolean;
    hasAnimelok: boolean;
    hasAnimeya: boolean;
    ...
    queryFn: async () => {
      ...
      let data: StreamingData & { hasAniStreamAPI: boolean };
      try {
        ...
        if (ENABLE_COMBINED_SOURCE_CACHE && cached && !isProviderSourceRefreshPending()) {
          return {
            ...cached,
            hasAniStreamProviders: (cached.providerServers?.length || 0) > 0,
      ...
      return {
        ...data,
        hasAniStreamProviders: (data.providerServers?.length || 0) > 0,
