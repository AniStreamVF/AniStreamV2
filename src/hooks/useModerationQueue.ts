...
export function useReviewModeration() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Élément de modération examiné');
    },
    onError: (error: Error) => {
      toast.error(error.message || "Échec de l'examen de l'élément");
    },
  });
}
...
export function useFlagContent() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['moderation_queue'] });
      queryClient.invalidateQueries({ queryKey: ['moderation_count'] });
      toast.success('Contenu signalé pour examen');
    },
    onError: (error: Error) => {
      toast.error(error.message || 'Échec du signalement du contenu');
    },
  });
}
