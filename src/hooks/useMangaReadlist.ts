...
import { mapAniStreamMangaStatusToAniList, updateAniListMangaStatus } from '@/lib/externalIntegrations';
...
const LOCAL_MANGA_READLIST_KEY = 'AniStream:manga-readlist:v1';
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (!variables.silentToast) {
        const action = data.status === 'plan_to_read' ? 'Ajouté à la liste de lecture' : 'Liste de lecture mise à jour';
        toast.success(action);
      }
    },
    onError: (error: any) => {
      toast.error(`Échec de la mise à jour de la liste de lecture : ${error?.message || 'Erreur inconnue'}`);
    },
  });
}
...
export function useRemoveFromMangaReadlist() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast.error(`Échec du retrait de la liste de lecture : ${error?.message || 'Erreur inconnue'}`);
    },
  });
}
