    ...
    server: {
      ...
      proxy: {
        ...
        '/api/v2/hianime': {
          ...
          changeOrigin: true,
          secure: false,
          rewrite: (p) => p.replace('/api/v2/hianime', ''),
        },
        ...
        // Dev proxy for all provider calls → local AniStreamCore (https://api.AniStream.me)
        '/api/providers': {
          target: backendOrigin,
          changeOrigin: true,
          secure: false,
        ...
        // Bridge server proxy endpoints (video/page proxy to avoid CORS)
        '/proxy': {
          target: backendOrigin,
          changeOrigin: true,
          secure: false,
