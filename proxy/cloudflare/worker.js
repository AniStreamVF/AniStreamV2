...
const CORS_ALLOW_HEADERS =
  'Content-Type, Authorization, X-Proxy-Password, X-AniStream-Proxy-Password, Range, Accept, Origin';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === '/health') {
      return jsonResponse({ ok: true, service: 'AniStream-streaming-proxy' }, 200, cors);
    }
...
function extractProxyPassword(request, requestUrl) {
  const headerPassword = String(
    request.headers.get('x-proxy-password') ||
      request.headers.get('x-AniStream-proxy-password') ||
      ''
  ).trim();
...
function buildPlaylistCacheRequest(targetUrl, referer, userAgent) {
  const keyUrl = new URL('https://AniStream-proxy-cache.local/playlist');
