...
describe("Provider Service Fixes", () => {
  ...
  describe("Console Logging Format", () => {
    it("should format provider aggregation logs correctly", () => {
      ...
      const logMessage = `[AniStream] Provider aggregation: ${providers.length} provider(s) returned 12 total source(s) | ${providerNames}`;
      
      expect(logMessage).toContain("[AniStream]");
    ...
    it("should handle empty provider case", () => {
      const providers = [];
      const providerNames = providers.map(g => g.displayName).join(", ");
      const logMessage = `[AniStream] Provider aggregation: ${providers.length} provider(s) returned 0 total source(s) | ${providerNames || "none"}`;
