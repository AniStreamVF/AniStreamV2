...
This document defines the visual language and design system for the AniStream web application, with a focus on the **Blossom (Cherry Blossom)** theme as the primary aesthetic.

## ✨ Core Philosophy
AniStream aims for a **Premium, Cinematic, and Dynamic** user experience. The Blossom theme specifically brings a soft, atmospheric, yet high-tech feel to the platform, inspired by the beauty of Sakura and modern cyberpunk aesthetics.
...
The signature UI element for AniStream.
```css
.glass-panel {
  position: relative;
  overflow: hidden;
