...
3. Start the server!
    ...
    Now the server should be running on [https://api.AniStream.me:4000](https://api.AniStream.me:4000)
...
The above command will start the server on port 4000. You can access the server at [https://api.AniStream.me:4000](https://api.AniStream.me:4000), and you can also change the port by changing the `-p` option to `-p <port>:4000`.
...
## Subtitle Management & Custom Sources

Enhanced subtitle support allows for mixing official tracks with community-provided or host-provided external tracks (VTT/SRT).

### 1. External Subtitle URLs
Database tables now support linking external subtitle files:
- **`custom_video_sources`**:
  - `subtitle_url` (text): URL to a .vtt or .srt file.
  - `subtitle_lang` (text): The language label for this track (e.g., "English", "Spanish Fan-sub").
- **`watch_rooms`**:
  - `manual_subtitle_url` (text): A host-provided URL that is live-synced to all participants in the room.

### 2. Subtitle Proxy (CORS Bypass)
To ensure subtitles work across different domains (CORS), use the `rapid-service` proxy:
- **Example Usage**:
  `GET /functions/v1/rapid-service?url={subtitle_url}&type=api`
- **Behavior**:
  - Fetches the subtitle file server-side.
  - Returns the content with permissive CORS headers.
  - The `VideoPlayer` further converts these into `blob:` URLs for guaranteed browser compatibility.
