...
const hianimeApiUrl = 'https://api.AniStream.me/api/v2/hianime';
const AniStreamApiUrl = process.env.AniStream_API_URL || 'https://api.AniStream.me/api/v1';
...
async function fetchFromEpisodeSources(episodeId) {
    try {
        ...
        const response = await fetch(`${AniStreamApiUrl}/hianime/episode/sources?animeEpisodeId=${encodeURIComponent(episodeId)}&server=hd-1&category=sub`);
