...
// Set app name to AniStream
app.setName('AniStream');
...
function getOrCreateCID() {
    const cidPath = path.join(app.getPath('userData'), '.AniStream-cid');
    try {
...
if (process.defaultApp) {
    if (process.argv.length >= 2) {
        app.setAsDefaultProtocolClient('AniStream', process.execPath, [path.resolve(process.argv[1])]);
    }
} else {
...
function createWindow() {
    ...
    const loadURL = async (url) => {
        try {
            ...
            } else if (isDev && url === 'https://api.AniStream.me:8088') {
                console.log('[Loading] Trying fallback port 8089...');
                await loadURL('https://api.AniStream.me:8089');
            } else {
    ...
    mainWindow.webContents.on('will-navigate', (event, url) => {
        ...
        if (url.startsWith('https://api.AniStream.me') || url.startsWith('file://')) {
            return;
        }

        // Block any navigation attempts from within the app (click hijacking)
        if (!currentURL.startsWith('https://api.AniStream.me') && !currentURL.startsWith('file://')) {
            return;
        }
...
ipcMain.handle('export-logs', async () => {
    const { canceled, filePath } = await dialog.showSaveDialog(mainWindow, {
        title: 'Export Logs',
        defaultPath: 'AniStream-logs.txt',
...
if (!gotTheLock) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        if (url && url.startsWith('AniStream://')) {
            handleDeepLink(url);
        }
    });
}
...
function handleDeepLink(url) {
    const path = url.replace('AniStream://', '/');
    if (mainWindow) {
...
async function setActivity(details, state, extra = {}) {
    ...
    const activity = {
        ...
        largeImageKey: 'logo',
        largeImageText: 'AniStream - Watch Anime Online',
        instance: false,
        buttons: [
            { label: 'Watch with me!', url: 'https://AniStream.me' }
        ]
    };
...
ipcMain.handle('get-downloads-dir', async (event, customPath) => {
    return customPath || path.join(app.getPath('videos'), 'AniStream');
});

ipcMain.handle('get-offline-library', async (event, customPath) => {
    const downloadPath = customPath || path.join(app.getPath('videos'), 'AniStream');
...
ipcMain.handle('import-video-files', async () => {
    ...
    const downloadPath = path.join(app.getPath('videos'), 'AniStream');
    if (!fs.existsSync(downloadPath)) {
        fs.mkdirSync(downloadPath, { recursive: true });
    }
    ...
    const { response, checkboxChecked } = await dialog.showMessageBox(mainWindow, {
        ...
        detail: 'Files will be copied to your AniStream library. You can organize them into anime folders later.',
    });
...
ipcMain.handle('reset-app-data', async () => {
    ...
    try {
        ...
        const defaultDownloadPath = path.join(app.getPath('videos'), 'AniStream');
