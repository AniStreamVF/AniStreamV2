...
serve(async (req) => {
  ...
  try {
    ...
    protectionScript.textContent = `
      // [AniStream Proxy] Ad-Shield v3 - Active Protection
      (function() {
        console.log("[AniStream] Ad-Shield active - neutralizing click hijacking");

        // 1. Override window.open to block popups
        const originalOpen = window.open;
        window.open = function(url, target, features) {
            console.log("[AniStream] Blocked popup:", url);
            return {
                closed: false,
                focus: function() {},
                blur: function() {},
        ...
        console.log("[AniStream Proxy] Active protection injected. Slug:", SERVER_DETECTED_SLUG);
      })();
    `;
